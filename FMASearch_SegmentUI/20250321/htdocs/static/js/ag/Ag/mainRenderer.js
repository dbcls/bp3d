...
//		console.log(self.__config);
		...
		self.__config.concurrencyPromise = Ext.isEmpty(self.__config.concurrencyPromise) ? 2 : (Ext.isNumeric(self.__config.concurrencyPromise) ? self.__config.concurrencyPromise - 0 : 2);
...
//			position: 'relative',
	...
	//	self._loadObjParams = [];
		...
		self.__target = new THREE.Vector3();
		self.__target.setScalar(0);
...
//			console.log('documentMouseUp');
			...
			if(Ext.isArray(mouseDownTranslate)){
//				console.log('documentMouseUp',mouseDownTranslate);
...
//		var height = self.__MAX_HEIGHT * (self.__aspectRatio<1 ?  self.__aspectRatio : 1);
	...
	getZoomScale : function(){
		var self = this;
//		console.log("getZoomScale()",self.__zoom.scale(), Math.round(Math.log(self.__zoom.scale()) / Math.log(2) * 10) / 10);
...
//			self.startAnimate();
	...
	setObjProperties : function(params){
		...
		if(self.__config.usePromise && Ext.isFunction(window.Promise)){
			if(Ext.isFunction(Promise.map)){
				Promise.map(self.__tempObjParams, Ext.bind(self._setObjPropertiesPromise,self)).then(function(){
					self._calcCameraPos();
					self.render();
				});
			}
...
//				Ext.get(self.__domElement).mask(Ext.String.format('Please wait... [{0}%]',0));
		...
		if(self.__config.usePromise && Ext.isFunction(window.Promise)){
			...
			if(Ext.isFunction(Promise.map)){
				Promise.map(self.__tempObjParams, Ext.bind(self._loadObjPromise,self), { concurrency: self.__config.concurrencyPromise }).then(function(){
					self.autoFocus();
...
//		self.zoomCB();
	...
	getHorizontal : function(angle){
		var self = this;
		return self.__longitude + 90;
	},
...
//		console.log(self.__camera.up);
	...
	setCameraPos : function(vec3){
		var self = this;
		self.__zoom.translate([vec3.x, vec3.y]);
		self.__zoom.scale(vec3.z);
		self.zoomCB();
	},
