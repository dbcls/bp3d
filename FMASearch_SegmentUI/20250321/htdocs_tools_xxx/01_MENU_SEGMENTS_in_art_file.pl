...
#	undef $ART2SEG if(defined $ART2SEG);
	...
	foreach my $art (@$ART){
		...
		my $art_data_size = $art->{art_data_size};
...
#		delete $ART2SEG->{$art_id} if($art_id eq 'MM7214');	# DEBUG
		...
		if(
					-e	$file
			&&	-f	$file
			&&	-s	$file == $art_data_size
			&&	defined	$ART2SEG
			&&	ref			$ART2SEG eq 'HASH'
			&&	exists	$ART2SEG->{$art_id}
			&&	defined	$ART2SEG->{$art_id}
			&&	ref			$ART2SEG->{$art_id} eq 'HASH'
			&&	scalar keys(%{$ART2SEG->{$art_id}}) > 0
		){
			print STDERR "\texists\n";
			next;
		}
		...
		unless(-e $file && -f $file && -s $file == $art_data_size){
			my $art_data;
