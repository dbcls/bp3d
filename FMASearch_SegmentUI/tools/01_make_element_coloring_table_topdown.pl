...
left join
 model_version as mv on mr.md_id=mv.md_id and mr.mv_id=mv.mv_id
where
 ...
 (
  ...
  from
   model_revision as mr
  left join (
   select md_id,mv_id,mv_use from model_version
...
SQL
	...
	my $mr_version;
	my $mv_name_e;
	my $model_revision_sth = $dbh->prepare($model_revision_sql) or die $dbh->errstr;
	$model_revision_sth->execute() or die $dbh->errstr;
	$column_number = 0;
	$model_revision_sth->bind_col(++$column_number, \$md_id,   undef);
	$model_revision_sth->bind_col(++$column_number, \$mv_id,   undef);
	$model_revision_sth->bind_col(++$column_number, \$mr_id,   undef);
	$model_revision_sth->bind_col(++$column_number, \$mr_version,   undef);
	$model_revision_sth->bind_col(++$column_number, \$mv_name_e,   undef);
	while($model_revision_sth->fetch){

		my $TEMP_FMA_HASH = &Clone::clone($FMA_HASH);
...
SQL
		...
		$buildup_tree_info_sth = $dbh->prepare(sprintf($buildup_tree_info_sql,join(',',map {'?'} keys(%$TEMP_FMA_HASH)))) or die $dbh->errstr;
		$buildup_tree_info_sth->execute(keys(%$TEMP_FMA_HASH)) or die $dbh->errstr;
		$column_number = 0;
		...
		while($buildup_tree_info_sth->fetch){
			unless(exists $TEMP_FMA_HASH->{$cdi_id} && defined $TEMP_FMA_HASH->{$cdi_id}){
				&cgi_lib::common::message("Unknown [$CDI_ID2NAME->{$cdi_id}]");
				next;
			}
			$TEMP_FMA_HASH->{$cdi_id}->{'but_depth'} = $but_depth - 0;

			$TEMP_FMA_HASH->{$cdi_id}->{'but_cids'} = undef;
			...
			if(defined $but_cids && ref $but_cids eq 'ARRAY' && scalar @$but_cids){
				$TEMP_FMA_HASH->{$cdi_id}->{'but_cids'}->{$_} = undef for(@$but_cids);
			}else{
...
#exit;
		...
		foreach my $cdi_id (keys(%$TEMP_FMA_HASH)){

			my $but_cids = $TEMP_FMA_HASH->{$cdi_id}->{'but_cids'};
			my $color_group = $TEMP_FMA_HASH->{$cdi_id}->{'color_group'};
			...
			if(defined $but_cids && ref $but_cids eq 'HASH' && scalar keys(%$but_cids)){
				foreach my $but_cid (keys(%$but_cids)){
					if(exists $TEMP_FMA_HASH->{$but_cid} && defined $TEMP_FMA_HASH->{$but_cid} && $TEMP_FMA_HASH->{$but_cid}->{'color_group'} ne $color_group && $TEMP_FMA_HASH->{$but_cid}->{'but_depth'} > $TEMP_FMA_HASH->{$cdi_id}->{'but_depth'}){
	#					&cgi_lib::common::message("exists\t$CDI_ID2NAME->{$but_cid}\t$cdi_pname\t$color_group\t$FMA_HASH->{$but_cid}->{'cdi_pname'}\t$FMA_HASH->{$but_cid}->{'color_group'}");
						if(exists $TEMP_FMA_HASH->{$but_cid}->{'but_cids'} && defined $TEMP_FMA_HASH->{$but_cid}->{'but_cids'} && ref $TEMP_FMA_HASH->{$but_cid}->{'but_cids'} eq 'HASH'){
							foreach (keys(%{$TEMP_FMA_HASH->{$but_cid}->{'but_cids'}})){
								delete $TEMP_FMA_HASH->{$cdi_id}->{'but_cids'}->{$_} if(exists $TEMP_FMA_HASH->{$cdi_id}->{'but_cids'}->{$_});
							}
						}
						delete $TEMP_FMA_HASH->{$cdi_id}->{'but_cids'}->{$but_cid} if(exists $TEMP_FMA_HASH->{$cdi_id}->{'but_cids'}->{$but_cid});
					}
				}
			}
		}
		...
		foreach my $cdi_id (keys(%$TEMP_FMA_HASH)){
			...
			my $color_group = $TEMP_FMA_HASH->{$cdi_id}->{'color_group'};
			my $color_group_id = $TEMP_FMA_HASH->{$cdi_id}->{'color_group_id'};
			my $cdi_pname = $CDI_ID2NAME->{$cdi_id};
			...
			if(defined $but_cids && ref $but_cids eq 'HASH' && scalar keys(%$but_cids)){
				foreach my $but_cid (keys(%$but_cids)){
					...
					if(exists $TEMP_FMA_HASH->{$but_cid} && defined $TEMP_FMA_HASH->{$but_cid} && $TEMP_FMA_HASH->{$but_cid}->{'color_group'} ne $color_group){
						&cgi_lib::common::message("exists\t$CDI_ID2NAME->{$but_cid}\t$cdi_pname\t$color_group\t$TEMP_FMA_HASH->{$but_cid}->{'cdi_pname'}\t$TEMP_FMA_HASH->{$but_cid}->{'color_group'}");
						next;
					}
					$TEMP_FMA_HASH->{$but_cid} = {
						cdi_pid => $cdi_id,
						cdi_pname => $cdi_pname,
						color_group => $color_group,
						color_group_id => $color_group_id
		...
		foreach my $cdi_id (sort {$TEMP_FMA_HASH->{$a}->{'color_group_id'} <=> $TEMP_FMA_HASH->{$b}->{'color_group_id'}} sort {$CDI_ID2NAME->{$a} cmp $CDI_ID2NAME->{$b}} keys(%$TEMP_FMA_HASH)){
			my $color_group = $TEMP_FMA_HASH->{$cdi_id}->{'color_group'};
			...
			$FMA_COLOR->{$mr_version
		}
	}
	$model_revision_sth->finish;
