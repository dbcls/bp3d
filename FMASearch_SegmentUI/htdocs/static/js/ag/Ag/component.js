...
//												minValue: -1,
	...
	_createViewport : function(){
		...
		var record = null;
		if(Ag.Def.DEF_MODEL_VERSION_TERM) record = version_store.findRecord('display',Ag.Def.DEF_MODEL_VERSION_TERM,0,false,false,true);
		...
		var make_ag_word = function(value,tooltip,className,baseClassName,isSelected){
...
//				selected_values = isSelected.concat([]);
				...
				isSelected = isSelected.length>0 ? true : false;
			}
			else{
				...
				Ext.Array.map(
					...
					function(str){
						if(Ext.isString(str) && str.length && str.match(/[a-z0-9]+/i)){
							...
							return Ext.util.Format.format(
								'<a href="#" class="{0} {1}" data-value="{2}" style="{3}">{4}</a>'
								,baseClassName
								,className
								,str.toLowerCase()
			...
			if(Ext.isString(value) && value.length){
				...
				Ext.Array.each(value, function(v,i){
					if(Ext.isString(v) && v.length){
						rtn.push(make_ag_word(v,null,'bp3d-relation'));
					}
					else if(Ext.isObject(v) && Ext.isString(v['name']) && v['name'].length){
						rtn.push(make_ag_word(v['name'],Ext.util.Format.format('{0} : {1}',v['id'],v['name']),'bp3d-relation'));
					}
				});
				return rtn.join('');
			}
...
//							var window_panel_toolbar = window_panel.getDockedItems('toolbar[dock="top"]')[0];
							...
							var searchfield = window_panel_toolbar.down('searchfield');
			...
			listeners: {
				afterrender: function(panel, eOpts){
					...
					search_store.on({
						...
						load: function(store,records,successful,eOpts){
							if(!successful) panel.setLoading(false);
						}
					});
					...
					var update_selected_pallet_rows = function(){
						...
						if(toolbar){
							...
							Ext.Array.each(toolbar.items.items, function(item){
								if(!item.isXType('button') || item.getItemId()=='select-all' || item.getItemId()=='deselect-all' || item.getItemId()=='items-title-checkboxfield') return true;
...
//												var node = gridview.getNode(record);
												...
												if(record.get(Ag.Def.CONCEPT_DATA_PICKED_DATA_FIELD_ID)){
													...
													if(Ext.isArray(word_values) && word_values.length){
														self._findWordRecords(store,Ag.Def.NAME_DATA_FIELD_ID,word_values,self,{callback:function(store,fieldName,value,re_value_arr,record){
...
//							.appendTo($td)
							.appendTo($bodydiv)
							.click(function(e){
								return self._clickSegmentTag($(this).attr('data-value'));
							})
							;
/*
							var $checked_div = $('<div>')
...
//							.appendTo($td)
							.appendTo($bodydiv)
							.click(function(e){
								return self._clickCategoryTag($(this).attr('data-value'));
							})
							;
					...
					columns: [
						{
							...
							renderer: function(value, metaData, record, rowIndex, colIndex, store, view){
								...
								if(Ext.isString(rtn) && rtn.length){
									return make_ag_word(rtn,value,'bp3d-fmaid');
								}
								return rtn;
...
//									return make_ag_word(value,null,'bp3d-name');
									...
									return make_ag_word(value,null,'bp3d-name',null,record.get(Ag.Def.CONCEPT_DATA_SELECTED_WORD_TAG_DATA_FIELD_ID));
								}
								return value;
							}
						},
						...
						,{
							...
							renderer: function(value, metaData, record, rowIndex, colIndex, store, view){
								...
								return value;
								if(Ext.isString(value) && value.length){
									return make_ag_word(value,null,'bp3d-sub');
								}
								return value;
							}
						}
						,{
							...
							renderer: function(value, metaData, record, rowIndex, colIndex, store, view){
								...
								return value;
								if(Ext.isString(value) && value.length){
									return make_ag_word(value,null,'bp3d-laterality');
								}
								return value;
							}
						}
	...
	//							dataIndex: Ag.Def.OBJ_CITIES_FIELD_ID+renderer_dataIndex_suffix,
							...
							renderer: function(value, metaData, record, rowIndex, colIndex, store, view){
								...
								return value;
								if(Ext.isString(value) && value.length){
									return make_ag_word(value,null,'bp3d-segment');
								}
								else if(Ext.isArray(value) && value.length){
									var rtn = [];
									Ext.Array.each(value, function(v,i){
										if(Ext.isString(value[i]) && value[i].length){
											rtn.push(make_ag_word(value[i],null,'bp3d-segment'));
										}
						...
						,{
							...
							renderer: function(value, metaData, record, rowIndex, colIndex, store, view){
								...
								return value;
								if(Ext.isString(value) && value.length){
									return make_ag_word(value,null,'bp3d-category');
								}
								return value;
							}
						}
	...
	//							tooltip: 'System',
							...
							renderer: function(value, metaData, record, rowIndex, colIndex, store, view){
								...
								if(Ext.isString(value) && value.length){
									...
									if(view.getItemId()==='selected-tags-tableview') return value;
									return value;
									return make_ag_word(value,null,'bp3d-system');
								}
								return value;
							}
						}
	...
	//								console.log(value, rowIndex, colIndex);
		...
		var cell_pick = function(records,gridpanel){
...
//														console.log(ren,intersects,e);
														...
														fieldcontainer.setDisabled(intersects.length==0);
														if(fieldcontainer.isDisabled()){
											...
											callback: function(records, operation, success){
												...
												if(success && records.length){
													...
													Ext.create('Ext.window.Window', {
														title: 'Neighbor Parts',
														height: match_list_gridpanel.getHeight(),
														width: match_list_gridpanel.getWidth()+60,
														modal: true,
														layout: 'fit',
														items: _createCanvasNeighborPartsPanel({title:null}),
														buttons: [{
															text: 'to Palette',
															listeners: {
																click: function(button){
																	var canvas_neighbor_parts_store = Ext.data.StoreManager.lookup('canvas-neighbor-parts-store');
																	var neighbor_gridpanel = button.up('window').down('gridpanel#canvas-neighbor-parts-gridpanel');
																	var neighbor_selmodel = neighbor_gridpanel.getSelectionModel();
																	var neighbor_records = neighbor_selmodel.getSelection();
																	if(Ext.isEmpty(neighbor_records)) neighbor_records = canvas_neighbor_parts_store.getRange();

																	var match_list_store = Ext.data.StoreManager.lookup(Ag.Def.CONCEPT_MATCH_LIST_STORE_ID);
																	var add_datas = [];
																	var remove_records = [];
																	Ext.Array.each(neighbor_records,function(neighbor_record){
																		if(Ext.isEmpty(match_list_store.findRecord(Ag.Def.ID_DATA_FIELD_ID, neighbor_record.get(Ag.Def.ID_DATA_FIELD_ID), 0, false, false, true))){
																			add_datas.push(neighbor_record.getData());
																			remove_records.push(neighbor_record);
																		}
																	});
																	if(add_datas.length){
																		canvas_neighbor_parts_store.remove(remove_records);
																		var add_records = match_list_store.add(add_datas);
																		match_list_store.sort();
																		var selmodel = match_list_gridpanel.getSelectionModel();
																		selmodel.deselectAll();
																		selmodel.select(add_records,false,false);
																		match_list_gridpanel.getView().focusRow(add_records[0]);
																	}
																}
															}
														},{
					...
					items: [{
						...
						listeners: {
							click: function(button){
								...
								if(toolbar){
									Ext.Array.each(toolbar.items.items, function(item){
										if(!item.isXType('button') || item.getItemId()=='select-all' || item.getItemId()=='deselect-all') return true;
										item.setDisabled(disabled);
									});
								}
							}
						...
						listeners: {
							click: function(button){
								...
								try{
									...
									if(Ext.isArray(records) && records.length) cell_pick(records,gridpanel);
...
//								return make_ag_word(value);
								console.log(value,record.get(Ag.Def.CONCEPT_DATA_SELECTED_WORD_TAG_DATA_FIELD_ID));
								return make_ag_word(value,null,null,null,record.get(Ag.Def.CONCEPT_DATA_SELECTED_WORD_TAG_DATA_FIELD_ID));
							}
							return value;
						}
					},
					...
					,{
						...
						renderer: function(value, metaData, record, rowIndex, colIndex, store, view){
							...
							if(Ext.isString(value) && value.length){
								...
								Ext.Array.each(value, function(v,i){
									if(Ext.isString(value[i]) && value[i].length){
										rtn.push(make_ag_word(value[i],null,'bp3d-segment',null,record.get(Ag.Def.CONCEPT_DATA_SELECTED_SEGMENT_TAG_DATA_FIELD_ID)));
									}
									else if(Ext.isNumber(value[i])){
										rtn.push(make_ag_word(value[i]+'',null,'bp3d-segment',null,record.get(Ag.Def.CONCEPT_DATA_SELECTED_SEGMENT_TAG_DATA_FIELD_ID)));
									}
...
//							tooltip: 'System',
						...
						renderer: function(value, metaData, record, rowIndex, colIndex, store, view){
							...
							if(Ext.isString(value) && value.length){
								if(value.match(/^[0-9]+(.+)$/)) value = RegExp.$1;
								if(value.match(/^_+(.+)$/)) value = RegExp.$1;
								return make_ag_word(value,null,'bp3d-category',null,record.get(Ag.Def.CONCEPT_DATA_SELECTED_CATEGORY_TAG_DATA_FIELD_ID));
							}
							return value;
						}
					}
...
*/
						var update_selected_rows = function(){
							...
							if(toolbar){
								...
								var count = Ext.Array.filter(store.getRange(), function(record){
						...
						gridpanel.addDocked([{
							...
							items: [{
								...
								store: Ext.create('Ext.data.ArrayStore', {
									fields: ['value','text'],
									data : [[Ag.Def.CONCEPT_DATA_SELECTED_TAG_DATA_FIELD_ID,'Tagged'],[Ag.Def.CONCEPT_DATA_SELECTED_PICK_DATA_FIELD_ID,'Picked']]
								}),
								displayField: 'text',
								valueField: 'value',
								queryMode: 'local',
...
//		console.log('refreshView()', view.ownerCt.getItemId());
	...
	_clickWordTag: function(value){
		...
		var $ad_hoc_label    = $this.find('label.ad-hoc-tag-label[data-value="'+value+'"]');
...
//				Ext.Array.each([pallet_store_records,match_list_store_records], function(records){
				Ext.Object.each(find_records, function(storeId,records){
					Ext.Array.each(records, function(record){
						...
						var selected = word_tag_data.length===values.length;
...
//								Ag.Def.CONCEPT_DATA_SELECTED_WORD_TAG_DATA_FIELD_ID
	...
	_clickSegmentTag: function(value){
		var self = this;
		var viewport = self.getViewport();
...
//										if(Ext.isObject(data) && Ext.isArray(data.element) && data.element.length){
	...
	_clickCategoryTag: function(value){
		var self = this;
//		var $this = $(tag_htmlelement);
		var viewport = self.getViewport();
		...
		var category_tag_panel   = window_panel.down('panel#category-tag-panel');
		var $this = $(category_tag_panel.body.dom).find('div.category-tag[data-value="'+value+'"]');
