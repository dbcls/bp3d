...
select
 cdi.cdi_id,
 cdi.cdi_name,
 COALESCE(cd.cd_name,bd.cd_name,cdi.cdi_name_e) as cd_name,
 cdi.cdi_name_j as cd_name_j,
 COALESCE(cd.cd_syn,bd.cd_syn,cdi.cdi_syn_e) as cd_syn,
 COALESCE(cd.cd_def,bd.cd_def,cdi.cdi_def_e) as cd_def,
 cdi.cdi_taid as cd_taid,
 COALESCE(cd.seg_color,bd.seg_color) as seg_color,
 COALESCE(cd.csg_name,bd.csg_name) as csg_name
from (
...
#		$sql = sprintf(qq|select cdi_id,but_pids from buildup_tree_info where md_id=$md_id and mv_id=$mv_id and mr_id=$mr_id and bul_id=0 and cdi_id in (%s)|,join(',',keys(%CD2ART)));
...
#						}
		...
		if(exists $config->{'verbose'}){
			my $bul_id;
#			$sql_fmt = qq|select cdi_id,but_depth,but_pids,but_cids,bul_id from buildup_tree_info where md_id=$md_id and mv_id=$mv_id and mr_id=$mr_id and bul_id<>0 and cdi_id in (%s) order by but_depth desc|;
#			$sql = sprintf($sql_fmt,join(',',keys(%CD2ART)));
