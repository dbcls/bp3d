...
//		name: 'cdi_name',
	...
	{
//		name: 'cdi_name',
		name: Ag.Def.ID_DATA_FIELD_ID+'_renderer',
		useNull: true,
		defaultValue: null,
		type: 'string',
		convert: function(value, record) {
			value = record.get(Ag.Def.ID_DATA_FIELD_ID);
			if(Ext.isString(value) && value.match(/^FMA([0-9]+)[A-Z]*\-[LRU]+$/)) value = RegExp.$1;
			if(Ext.isString(value) && value.match(/^FMA([0-9]+)\-.+$/)) value = RegExp.$1;
			if(Ext.isString(value) && value.match(/^FMA([0-9]+)$/)) value = RegExp.$1;
			return value;
		}
	},
...
Ext.define('Ag.data.Model.SYSTEM_TREE', {
	extend: 'Ext.data.TreeModel',
...
Ext.define('Ag.data.Model.NEIGHBOR_PARTS', {
	...
	{
		name: Ag.Def.EXISTS_PALETTE_FIELD_ID,
		defaultValue: false,
		type: 'boolean'
	}
	]
...
Ext.define('Ag.data.Model.KEYWORD_SEARCH', {
	extend: 'Ag.data.Model.CONCEPT_TERM_PALLET',
	fields: [
	{
		name: Ag.Def.EXISTS_PALETTE_FIELD_ID,
...
Ext.define('Ag.data.Model.SEGMENT_GRID', {
	extend: 'Ag.data.Model.CONCEPT_TERM_PALLET',
	fields: [
	{
		name: Ag.Def.EXISTS_PALETTE_FIELD_ID,
...
Ext.define('Ag.data.Model.SYSTEM_GRID', {
	extend: 'Ag.data.Model.CONCEPT_TERM_PALLET',
	fields: [
	{
		name: Ag.Def.EXISTS_PALETTE_FIELD_ID,
