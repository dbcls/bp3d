...
#		&cgi_lib::common::message($infer_crl_id, $LOG);
		...
		if(defined $temp_crl_id && $temp_crl_id == 4){
			my $partof_pids;
#			my $sth_ct_sel = $dbh->prepare("SELECT crt_ids FROM concept_tree WHERE ci_id=$ci_id AND cb_id=$cb_id AND cdi_id=$cdi_pid AND crl_id=$temp_crl_id GROUP BY crt_ids") or die $dbh->errstr;	#20190131 変更
			my $sth_ct_sel = $dbh->prepare("SELECT crt_id FROM concept_relation_type WHERE crt_name='regional_part_of'") or die $dbh->errstr;
			$sth_ct_sel->execute() or die $dbh->errstr;
			if($sth_ct_sel->rows()>0){
				$column_number = 0;
				my $relation_crt_ids;
...
--LEFT JOIN
-- concept_data_info AS cdip ON cm.ci_id=cdip.ci_id AND cdip.cdi_name=regexp_replace(cdi.cdi_name, '^(FMA[0-9]+).*\$', E'\\\\1')
LEFT JOIN
 concept_data AS cd ON cd.ci_id=cdi.ci_id AND cd.cdi_id=cdi.cdi_id
WHERE
 cdi.is_user_data
 AND cm.cm_use
...
#		&cgi_lib::common::message(\%HASH_ALL_USE_LIST, $LOG) if(defined $LOG);
