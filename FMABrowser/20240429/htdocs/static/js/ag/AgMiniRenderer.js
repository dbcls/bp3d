...
//	self.SCREEN_HEIGHT = 60;
	...
	self.$_domElement = $('<div style="width:'+(self.SCREEN_WIDTH)+'px;height:'+(self.SCREEN_HEIGHT)+'px;background-color:#f0f0f0;border:1px solid #b5b8c8;position:relative;text-align:center;">');
...
//	},
	...
	setSize : function(width,height){
		...
		self.$_domElement.width(self.SCREEN_WIDTH).height(self.SCREEN_HEIGHT);
		self.SCREEN_HEIGHT -= 2;
		self.SCREEN_WIDTH  -= 2;

		if(self.SCREEN_WIDTH < self.SCREEN_HEIGHT){
			self._renderer.setSize( self.SCREEN_WIDTH, self.SCREEN_WIDTH );
		}else{
			self._renderer.setSize( self.SCREEN_HEIGHT, self.SCREEN_HEIGHT );
		}
		self.focus();

//		self._camera.aspect = self.SCREEN_WIDTH / self.SCREEN_HEIGHT;
//		console.log(self._camera, self.SCREEN_WIDTH / self.SCREEN_HEIGHT);
/*
		self._camera.left   = -self.SCREEN_WIDTH  / self._camFactor;
		self._camera.right  =  self.SCREEN_WIDTH  / self._camFactor;
		self._camera.top    =  self.SCREEN_HEIGHT / self._camFactor;
		self._camera.bottom = -self.SCREEN_HEIGHT / self._camFactor;
