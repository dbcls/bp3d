...
#my $sth_cdi = $dbh->prepare(qq|SELECT cdi_id FROM concept_data_info WHERE is_user_data=false AND cdi_delcause IS NULL AND ci_id=$ci_id AND cdi_name=?|) or die $dbh->errstr;
...
if(defined $cdi_id){
#	$sth_cdi = $dbh->prepare(qq|select cdi_name,cd_name from concept_data as cd left join(select ci_id,cdi_id,cdi_name,is_user_data from concept_data_info) as cdi on cdi.ci_id=cd.ci_id and cdi.cdi_id=cd.cdi_id where is_user_data=false AND cd_delcause is null and cd.ci_id=$ci_id and cd.cb_id=$cb_id and cd.cdi_id=?|) or die $dbh->errstr;
	$sth_cdi = $dbh->prepare(qq|select cdi_name,cd_name from concept_data as cd left join(select ci_id,cdi_id,cdi_name,is_user_data from concept_data_info) as cdi on cdi.ci_id=cd.ci_id and cdi.cdi_id=cd.cdi_id where cd_delcause is null and cd.ci_id=$ci_id and cd.cb_id=$cb_id and cd.cdi_id=?|) or die $dbh->errstr;
...
#	my $sth_ct_down = $dbh->prepare(qq|select ct.cdi_id,ct.bul_id,ct.f_potids from concept_tree as ct where ct.ci_id=$ci_id and ct.cb_id=$cb_id and ct.cdi_pid=? and ct.cdi_id is not null and ct.bul_id in ($bul_ids)|) or die $dbh->errstr;
	...
	if(&existsTable('fma_partof_type')){
		$sql_ct_up = qq|select ct.cdi_pid,ct.bul_id,ct.f_potids from concept_tree as ct where ct.ci_id=$ci_id and ct.cb_id=$cb_id and ct.cdi_id=? and ct.cdi_pid is not null and ct.bul_id in ($bul_ids)|;
		$sql_ct_down = qq|select ct.cdi_id,ct.bul_id,ct.f_potids from concept_tree as ct where ct.ci_id=$ci_id and ct.cb_id=$cb_id and ct.cdi_pid=? and ct.cdi_id is not null and ct.bul_id in ($bul_ids)|;
	}else{
		$sql_ct_up = qq|select ct.cdi_pid,ct.crl_id,ct.crt_ids from concept_tree as ct where ct.ci_id=$ci_id and ct.cb_id=$cb_id and ct.cdi_id=? and ct.cdi_pid is not null and ct.crl_id in ($bul_ids)|;
		$sql_ct_down = qq|select ct.cdi_id,ct.crl_id,ct.crt_ids from concept_tree as ct where ct.ci_id=$ci_id and ct.cb_id=$cb_id and ct.cdi_pid=? and ct.cdi_id is not null and ct.crl_id in ($bul_ids)|;
	}
