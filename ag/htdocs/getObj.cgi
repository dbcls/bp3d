...
my $size = $FORM{s} eq 'L' ? '640x640' : '120x120';
...
#&print_path();
&print_path() unless(defined $angle);
...
if(-e $obj_path){
	...
	if(!-e $param_file && mkdir($lock_file)){
		...
		if(defined $angle){
			system(qq|./obj2image.pl $param_file|);
			...
			if(defined $pid){
				if($pid == 0){
					my $f1 = "tmp_image/obj2image.log";
					my $f2 = "tmp_image/obj2image.err";
					close(STDOUT);
					close(STDERR);
					open STDOUT, ">> $f1" || die "[$f1] $!\n";
					open STDERR, ">> $f2" || die "[$f2] $!\n";
					close(STDIN);
					system(qq|nice -n 19 ./obj2image.pl $param_file|);
					rmdir $lock_file if(-e $lock_file);
					unlink $param_file if(-e $param_file);
					exit($?);
				}else{
