...
my $img_prefix = &getRepImagePrefix($FORM{i},$tg_model,$FORM{p},$size,$FORM{v},$FORM{c});
my $img_extension = &getRepImageFileExtension($FORM{p});
my $img_dir = &getRepImageDir($FORM{i},$tg_model);
...
my $obj_path = &getRepObjsPath($dbh,$FORM{i});
print $LOG __LINE__,qq|:\$obj_path=[$obj_path]\n|;
if(defined $obj_path){
	my $img_basedir = &getRepImageBaseDir();
	my $img_prefix = &getRepImagePrefix($FORM{i},$tg_model,'rotate',$size,$FORM{v},$FORM{c});
