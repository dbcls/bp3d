...
#my $sth_isa = $dbh->prepare(qq|select fma_isa.f_id  from fma_isa where fma_isa.f_pid=?|);
my $sql =<<SQL;
SELECT
 i1.cdi_name
FROM concept_tree
...
WHERE concept_tree.ci_id = $FORM{'ci_id'} AND concept_tree.bul_id=(select bul_id from buildup_logic where bul_rep_key='I') AND concept_tree.cb_id = $FORM{'cb_id'} and i2.cdi_name=?
SQL
...
$f_id =~ s/\D+$//g;
$sth_isa->execute($f_id);
my $f_cid;
my $column_number = 0;
$sth_isa->bind_col(++$column_number, \$f_cid, undef);
while($sth_isa->fetch){
