...
use FindBin;
use lib $FindBin::Bin,qq|$FindBin::Bin/..|,qq|$FindBin::Bin/../IM|,qq|$FindBin::Bin/../../../ag-common/lib|;
#use lib $FindBin::Bin,qq|/bp3d/ag/htdocs_130624|,qq|/bp3d/ag/htdocs_130624/IM|;
...
unless(DEBUG){
	...
	open($LOG,">> $FindBin::Bin/logs/$cgi_name\_$log_time.txt");
...
=pod
	...
	if(defined $r){
		print $LOG __LINE__,":",Dumper($r);
		if(defined $r->{'results'}){
			push(@$results,@{$r->{'results'}});
		}elsif(defined $r->{'statuses'}){
			push(@$results,@{$r->{'statuses'}});
		}
...
my $res = &yahoo::get();
push(@$results,@$res) if(defined $res && ref $res eq 'ARRAY');

#my $dbh = &get_dbh();
my $Twitter = new AG::ComDB::Twitter;
my $dbh = $Twitter->get_dbh();
...
=pod
		if($rows>=1){#登録済みの場合
			my $sql = qq|delete from twitter_search where tw_id=?|;
			my $sth = $dbh->prepare($sql);
			$sth->execute($result->{'id'});
			$sth->finish;
			undef $sth;
...
SQL
			...
			$sth->bind_param(++$param_num, defined $result->{'lang'} ? $result->{'lang'} : 'ja');
...
my $tw_raw_data;
my $sql = qq|select tw_raw_data from twitter_search|;
my $sth = $dbh->prepare($sql);
$sth->execute();
my $column_number = 0;
$sth->bind_col(++$column_number, \$tw_raw_data, undef);
while($sth->fetch){
	next unless(defined $tw_raw_data);

	&utf8::encode($tw_raw_data) if(&utf8::is_utf8($tw_raw_data));

	my $result = &JSON::XS::decode_json($tw_raw_data);
	&insert($dbh,$result);
}
$sth->finish;
undef $sth;
...
sub insert {
	...
	if(defined $result->{'entities'} && defined $result->{'entities'}->{'urls'}){
		foreach my $url (@{$result->{'entities'}->{'urls'}}){
			next unless(defined $url);
			if(exists $url->{'expanded_url'} && $url->{'expanded_url'} =~ /\?shorten=([A-Za-z0-9]+)/){
				$shorten = $1;
				undef $shorten unless($url->{'expanded_url'} =~ qr|http://lifesciencedb.jp|);
			}
			if(exists $url->{'url'} && $url->{'url'} =~ /\?shorten=([A-Za-z0-9]+)/){
				$shorten = $1;
				undef $shorten unless($url->{'url'} =~ qr|http://lifesciencedb.jp|);
			}
			last if(defined $shorten);
		}
	}
	unless(defined $shorten){
		if(defined $result->{'text'} && length($result->{'text'}) && $result->{'text'} =~ /\?shorten=([A-Za-z0-9]+)/){
			$shorten = $1;
			undef $shorten unless($result->{'text'} =~ qr|http://lifesciencedb.jp|);
		}
	}
	if(defined $shorten){
	...
	if(defined $sp_original && $sp_original =~ /i=([A-Z]+[0-9]+)/){#対応レコードが無い場合は、ここのサイトから投稿されたデータでは無い
		my $rep_id = $1;

		my $sql = qq|select * from representation_twitter where rep_id=? and tw_id=?|;
		my $sth = $dbh->prepare($sql);
