...
my $bul_id;
my $but_cids;
my $sql = 'select cdi_id from buildup_tree where ci_id=? and cb_id=? group by cdi_id HAVING count(cdi_id)>1';
...
my %FMA20394;
$sql = 'select cdi_id,but_cids from view_buildup_tree where ci_id=? and cb_id=? and cdi_name=? and bul_id=? order by cdi_id,bul_id';
...
foreach $cdi_id (sort {$a<=>$b} keys(%CDI_ID)){
	my @bul_ids = sort {$a<=>$b} keys(%{$CDI_ID{$cdi_id}});
	next unless(scalar @bul_ids == 2);
	next if(scalar @{$CDI_ID{$cdi_id}->{$bul_ids[0]}} == scalar @{$CDI_ID{$cdi_id}->{$bul_ids[1]}});
	next if(scalar @{$CDI_ID{$cdi_id}->{$bul_ids[0]}} == 1);
	next if(scalar @{$CDI_ID{$cdi_id}->{$bul_ids[1]}} == 1);

	my $art_ids_3 = 0;
	my $art_ids_4 = 0;

	if(exists $ART_ID{$cdi_id}){
		$art_ids_3 = scalar @{$ART_ID{$cdi_id}->{$bul_ids[0]}} if(exists $ART_ID{$cdi_id}->{$bul_ids[0]});
		$art_ids_4 = scalar @{$ART_ID{$cdi_id}->{$bul_ids[1]}} if(exists $ART_ID{$cdi_id}->{$bul_ids[1]});
	}

	say $CDI{$cdi_id}->{'cdi_name'}."\t".$CDI{$cdi_id}->{'cdi_name_e'}."\t".(scalar @{$CDI_ID{$cdi_id}->{$bul_ids[0]}})."\t".(scalar @{$CDI_ID{$cdi_id}->{$bul_ids[1]}})."\t$art_ids_3\t$art_ids_4";
}
