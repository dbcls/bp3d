...
#	&cgi_lib::common::message($INFER);
	...
	$sql = qq|select * from concept_tree where ci_id=$ci_id AND cb_id=$cb_id AND bul_id=4 AND cdi_id=? AND cdi_pid=?|;
	$sth = $dbh->prepare($sql) or die $dbh->errstr;
...
print STDERR __LINE__.':ANALYZE concept_tree_trio;'."\n";
$dbh->do(qq|ANALYZE concept_tree_trio;|) or die $!;
