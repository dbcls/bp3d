...
SQL
	...
	while($sth->fetch){
		warn sprintf("             %-2d :            %3d : %-5s : %-25s : %10s\n",$ci_id,$cb_id,$ci_name,$cb_name,$cb_comment);
	}
	$sth->finish;
	undef $sth;
	exit 1;
...
eval{
	...
	$dbh->do(qq|update buildup_data set seg_id=0 where ci_id=$ci_id and cb_id=$cb_id|) or die $dbh->errstr;
	my $sql_cd = qq|update concept_data set seg_id=? where ci_id=? and cb_id=? and cdi_id in (%s) and seg_id=0|;
	my $sql_bd = qq|update buildup_data set seg_id=? where ci_id=? and cb_id=? and cdi_id in (%s) and seg_id=0|;

=pod
#	my %ID2COLOR;
	my $sql_but = qq|select cdi_id,but_cids from view_buildup_tree where ci_id=? and cb_id=? and cdi_name=?|;
	...
	foreach my $seg_name (sort keys(%DEF_SEG)){
		...
		foreach my $cdi_name (@{$DEF_SEG{$seg_name}}){
			...
			while($sth_but->fetch){
				...
				my $sth_cd = $dbh->prepare(sprintf($sql_cd,join(",",@$but_cids)));
...
=cut
	...
	my $group2color_file = '/bp3d/FMASearch/tools/group2color.txt';
	my $element_coloring_table_file = '/bp3d/FMASearch/tools/element_coloring_table.txt';
	if(
...
SQL
	...
	if(defined $FMA_HASH && ref $FMA_HASH eq 'HASH' && defined $GROUP2SEGID && ref $GROUP2SEGID eq 'HASH'){
		...
		$sth_but->bind_col(++$column_number, \$but_cids, undef);
		...
		while($sth_but->fetch){
			...
			my $sth_cd = $dbh->prepare(sprintf($sql_cd,join(',',@$but_cids))) or die $dbh->errstr;
		...
		$sth_but = $dbh->prepare(sprintf(qq|select cdi_id,but_cids,but_depth from buildup_tree_info where ci_id=$ci_id and cb_id=$cb_id and bul_id=3 and cdi_id in (%s) order by but_depth desc|,join(',',keys(%$FMA_HASH)))) or die $dbh->errstr;
		$sth_but->execute() or die $dbh->errstr;
		$column_number = 0;
		$sth_but->bind_col(++$column_number, \$cdi_id, undef);
		$sth_but->bind_col(++$column_number, \$but_cids, undef);
		...
		while($sth_but->fetch){
			...
			my $sth_cd = $dbh->prepare(sprintf($sql_bd,join(',',@$but_cids))) or die $dbh->errstr;
			$sth_cd->execute($seg_id,$ci_id,$cb_id) or die $dbh->errstr;
			&cgi_lib::common::message(sprintf('GROUP:[%-10s] : NAME:[%-10s] : ID:[%6d] : SEG:[%2d] : DEPTH:[%2d] : NUM:[%4d]',$color_group,$CDI_ID2NAME->{$cdi_id},$cdi_id,$seg_id,$cti_depth,$sth_cd->rows()));
			$sth_cd->finish;
			undef $sth_cd;
...
#		exit;
		...
		$column_number = 0;
		$sth_seg->bind_col(++$column_number, \$seg_id, undef);
		$sth_seg->bind_col(++$column_number, \$cdi_ids, undef);
		while($sth_seg->fetch){
			next unless(defined $cdi_ids);
			$cdi_ids = &JSON::XS::decode_json($cdi_ids);
			if(defined $cdi_ids && ref $cdi_ids eq 'ARRAY'){
				my $sql_but = sprintf(qq|select cdi_id,cti_cids from concept_tree_info where ci_id=$ci_id and cb_id=$cb_id and cdi_id in (%s)|, join(',',@$cdi_ids));
				my $sth_but = $dbh->prepare($sql_but);
				$sth_but->execute();
				my $cdi_id;
				my $but_cids;
