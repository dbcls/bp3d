...
//		height: 435
	...
	var hidden_info_takeover = true;
	var hidden_merge = true;
	...
	var setDisabledSaveButton = function(field){
		...
		formPanel.getDockedItems('toolbar[dock="bottom"]')[0].down('button').setDisabled(disable);
							...
							var dataset_mng_win = Ext.create('Ext.window.Window', {
								...
								layout: 'border',
								stateful: true,
								stateId: 'dataset-mng-window',
								items: [{
									region: 'center',
									xtype: 'aggridpanel',
									store: 'datasetMngStore',
									stateful: true,
									stateId: 'dataset-mng-gridpanel',
									columns: [
										{
											text: AgLang.model,
											dataIndex: 'md_name_e',
										...
										listeners: {
											click: function(b){
												...
												if(rec){
														...
														var conceptIdx = conceptBuildStore.findBy(function(record,id){
															if(record.get('ci_id')==rec.data.ci_id && record.get('cb_id')==rec.data.cb_id){
																return true;
															}else{
																return false;
															}
														...
														var conceptRec;
														if(conceptIdx>=0){
															conceptRec = conceptBuildStore.getAt(conceptIdx);
														}else{
															conceptRec = conceptBuildStore.getAt(0);
														}
...
*/
													if(conceptRec){
														n.set('ci_id',conceptRec.data.ci_id);
														n.set('ci_name',conceptRec.data.ci_name);
														n.set('cb_id',conceptRec.data.cb_id);
														n.set('cb_name',conceptRec.data.cb_name);
														n.set('cb_comment',conceptRec.data.cb_comment);
														n.set('cb_release',conceptRec.data.cb_release);
														n.set('cb_display',conceptRec.data.display);
														n.set('concept',conceptRec.data.value);
													}

													if(hidden_info_takeover){
														n.set('info_takeover',false);
													}else{
													...
													n.set('mr_version',version+'.'+revision+'.'+Ext.Date.format(new Date,'ymdHi'));

													n.endEdit(false,['version','version.version','version.revision','md_id','mv_id','prev_mv_id','mr_id','mv_frozen','mv_publish','mv_order','mv_port','ci_id','ci_name','cb_id','cb_name','cb_comment','cb_release','cb_display','concept','info_takeover','version_version','version_revision','fmt_version','mv_objects_set','mr_version']);
...
//													});
										...
										hidden: hidden_merge,
										text: AgLang.merge,
										iconCls: 'pallet_merge',
										listeners: {
											click: function(b){
												self.openDatasetMergeWindow({
													iconCls: b.iconCls,
													title: b.text || b.tooltip,
													animateTarget: b.el,
//													records: Ext.data.StoreManager.lookup('datasetMngStore').getRange().filter(function(record){ return record.get('mv_use'); })
													dataset_datas: Ext.data.StoreManager.lookup('datasetMngStore').getRange().map(function(r){return r.getData();}),
													concept_datas: Ext.data.StoreManager.lookup('conceptBuildStore').getRange().map(function(r){return r.getData();})
												});
											}
										}
									},{
...
//												version.setMinValue(parseInt(records[0].data.version));
												...
												readOnly = records[0].get('mv_frozen');
...
//												form.findField('mv_order').setDisabled(!readOnly);
												...
												if(readOnly){
													form.findField('cb_display').show();
													form.findField('concept').hide();
												}else{
													form.findField('cb_display').hide();
													form.findField('concept').show();
												}
...
//									height: 240,
//									height: 266,
									height: hidden_info_takeover ? 234 : 266,
									xtype: 'form',
									bodyPadding: 10,
									defaults: {
										labelAlign: 'right',
									...
									items: [{
										...
										listeners: {
											...
											select: function(field, records, eOpts){
												...
												rec.set('cb_display',record.data.display);
												rec.set('concept',record.data.value);
												rec.endEdit(false,[name,'ci_id','ci_name','cb_id','cb_name','cb_comment','cb_release','cb_display','concept']);

												var formPanel = field.up('form');
												var form = formPanel.getForm();
												form.findField('cb_display').setValue(record.data.display);
...
//															console.log('success()');
															...
															if(reader && reader.rawData){
																if(reader.rawData.msg && Ext.isString(reader.rawData.msg)){
																	Ext.Msg.show({
																		title: b.text,
																		iconCls: b.iconCls,
	//																	msg: msg,
																...
																if(reader.rawData.sessionID && Ext.isString(reader.rawData.sessionID)){
																	...
																	_progress.updateProgress(0,'0%',AgLang.save);
																	_progress.center();

																	var p = {};
																	p.cmd = 'update-progress';
																	p.sessionID = reader.rawData.sessionID;
//																	console.log(p);
																			...
																			Ext.Ajax.request({
																				...
																				callback: function(options,success,response){
																					...
																					if(json && json.progress){
																						if(Ext.isString(json.progress.msg) && json.progress.msg.toLowerCase()=='end'){
																							_progress.close();
																							task.destroy();
																						}
																						else if(Ext.isString(json.progress.msg) && json.progress.msg.toLowerCase().indexOf('error')==0){
																							Ext.defer(function(){
																								Ext.Msg.show({
																									title: AgLang.save,
