...
my $params_txt_file = &catfile($out_path,qq|$time_md5.txt|);
if(exists $PARAMS{'sessionID'} && defined $PARAMS{'sessionID'} && length $PARAMS{'sessionID'}){
	...
	$RTN->{'cb_id'} = $PARAMS{'cb_id'} - 0;
	$RTN->{'md_id'} = $PARAMS{'md_id'} - 0;
	$RTN->{'mv_id'} = $PARAMS{'mv_id'} - 0;
	$RTN->{'mr_id'} = $PARAMS{'mr_id'} - 0;
	$RTN->{'cmd'} = $PARAMS{'cmd'};
	&cgi_lib::common::writeFileJSON($params_file,$RTN);
	&cgi_lib::common::writeFileJSON($params_txt_file,$RTN,1);
}
...
if(defined $file_fh){
	...
	$upload_temp_file = &catfile($out_path,"$name$ext");
	&File::Copy::copy($upload_file,$upload_temp_file);
}
$RTN->{'file'} = $upload_file;
&cgi_lib::common::writeFileJSON($params_file,$RTN);

$RTN->{'file'} = $upload_temp_file;
&cgi_lib::common::writeFileJSON($params_txt_file,$RTN,1);
