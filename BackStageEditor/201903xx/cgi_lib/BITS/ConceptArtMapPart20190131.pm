package BITS::ConceptArtMapPart20190131;
...
#		my $sth_cmp_abbr_sel = $dbh->prepare(qq|SELECT cmp_abbr FROM concept_art_map_part WHERE cmp_id=?|) or die $dbh->errstr;
		my $sth_cmp_abbr_sel = $dbh->prepare(qq|SELECT cmp_abbr FROM concept_art_map_part WHERE md_id=? AND mv_id=? AND mr_id=? AND cmp_id=?|) or die $dbh->errstr;

		$sth_cmp_abbr_sel->execute($FORM{'md_id'},$FORM{'mv_id'},$FORM{'mr_id'},$cmp_id) or die $dbh->errstr;
...
SELECT
 ...
 COALESCE(cd.phy_id,bd.phy_id,0)
FROM
...
#	unless(defined $cdi_name_e && length $cdi_name_e){
	...
	undef $sth_cd_sel;
	unless($cd_rows>0){
		my $sth_cd_ins = $dbh->prepare("INSERT INTO buildup_data (md_id,mv_id,mr_id,ci_id,cb_id,cdi_id,cd_name,cd_entry,cd_openid,seg_id,phy_id,cmp_id) VALUES ($md_id,$mv_id,$mr_id,$ci_id,$cb_id,$cdi_id,?,now(),'system'::text,$seg_id,$phy_id,$cmp_id)") or die $dbh->errstr;
		$sth_cd_ins->execute($cd_name) or die $dbh->errstr;
		$sth_cd_ins->finish;
		undef $sth_cd_ins;
	}
	{
		$seg_id = 0 unless(defined $seg_id);
		$phy_id = 0 unless(defined $phy_id);
		my $sth_cd_upd = $dbh->prepare("UPDATE buildup_data SET cd_name=?,seg_id=?,phy_id=?,cmp_id=? WHERE md_id=$md_id AND mv_id=$mv_id AND mr_id=$mr_id AND cdi_id=?") or die $dbh->errstr;
		$sth_cd_upd->execute($cd_name,$seg_id,$phy_id,$cmp_id,$cdi_id) or die $dbh->errstr;
		$sth_cd_upd->finish;
		undef $sth_cd_upd;
	}
