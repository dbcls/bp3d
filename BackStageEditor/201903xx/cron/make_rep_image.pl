...
sub local_fork {
	my $pid;
#	$pid = fork;
#	warn __LINE__,":Cannot fork: $!" unless defined $pid;
	return $pid;
}

sub composite_bulk {
	my $angle = shift;
	my $target_png_fmt = shift;
	my $larger_png_fmt = shift;
	my $angle_png_fmt = shift;
	my $larger_size = shift;
	my $png_file = sprintf($angle_png_fmt,$angle);
	return if(-e $png_file && -s $png_file);
	my $target_png_file = sprintf($target_png_fmt,$angle);
	my $larger_png_file = sprintf($larger_png_fmt,$angle);
	return unless(-e $target_png_file && -s $target_png_file && -e $larger_png_file && -s $larger_png_file);
...
sub load_art_file_fromDB {
	...
	my $hist_serial = $args{'hist_serial'};
...
from
 history_art_file
where
 art_id=? AND hist_serial=?
SQL
	my $sth = $dbh->prepare($sql) or die $dbh->errstr;
	$sth->execute($art_id,$hist_serial) or die $dbh->errstr;
	...
	if(defined $obj_files1 && ref $obj_files1 eq 'ARRAY' && defined $obj_files2 && ref $obj_files2 eq 'ARRAY'){
		...
		foreach my $objfile (@FILES){
			if(ref $objfile eq 'HASH'){
				&load_art_file_fromDB(art_id=>$objfile->{'art_id'},hist_serial=>$objfile->{'hist_serial'}) unless(-e $objfile->{'path'});
   ...
   where
    rep_id in (
      ...
      where
       ...
       (md_id,mv_id,mr_id,cdi_id,bul_id) in (
           select
            md_id,mv_id,max(mr_id) as mr_id,cdi_id,bul_id
           from
            representation
           where
            md_id=$md_id and mv_id=$mv_id and mr_id<=$mr_id
           group by
            md_id,mv_id,cdi_id,bul_id
        )
    )
   group by
...
#	$sth->bind_col(++$column_number, \$art_data, { pg_type => DBD::Pg::PG_BYTEA });
	...
	while($sth->fetch){
		...
		$mtime = (stat($objfile))[9] if(-e $objfile);
		unless(-e $objfile && $size == $art_data_size && $mtime>=$art_entry){
			&load_art_file_fromDB(art_id=>$art_id,art_id=>$hist_serial);
		}
		$bp3d_objs->objReader($objfile,$reduction) if(-e $objfile);
	}
	$sth->finish;
...
#			die __LINE__,":\n";
		...
		if(USE_BP3D_COLOR){
			...
			my $sql_cm = qq|select art_id,art_hist_serial,cdi_name,cdi_id from view_concept_art_map WHERE cm_use AND cm_delcause IS NULL AND (md_id,mv_id,mr_id,cdi_id) IN (select md_id,mv_id,max(mr_id) as mr_id,cdi_id from concept_art_map where md_id=? AND mv_id=? AND mr_id<=? group by md_id,mv_id,cdi_id)|;
          ...
          where
           ...
           (md_id,mv_id,mr_id,cdi_id) in (
               ...
               from
                view_representation
               where
                md_id=? and mv_id=? and mr_id<=? and cdi_name=?
               group by
                md_id,mv_id,cdi_id
            )
      )
          ...
          where
           ...
           (md_id,mv_id,mr_id,bul_id,cdi_id) in (
               ...
               from
                view_representation
               where
                md_id=? and mv_id=? and mr_id<=? and bul_id=? and cdi_name=?
               group by
...
SQL
			...
			if(defined $change_cdi_ids){
				...
				foreach my $cdi_id (keys(%$change_cdi_ids)){
					next unless(exists $change_cdi_ids->{$cdi_id}->{$t_type});
					...
					push(@B_IDS,{
						cdi_name     => $cdi_name,
						rep_delcause => (defined $change_cdi_ids->{$cdi_id}->{$t_type} && $change_cdi_ids->{$cdi_id}->{$t_type} =~ /^delete/) ? 'DELETE' : undef,
						md_abbr      => $md_abbr,
						mr_version   => $version,
						rep_entry    => time,
						rep_ci_id    => $ci_id,
...
#				print STDERR __LINE__,":",Dumper(\@B_IDS),"\n";
#				die __LINE__,":\n";
			}
#				die __LINE__,":\n";
...
#				my $cache_json_base_path = sprintf(qq|/bp3d/cache_fma/%s/common|,$mr_version);
				my $cache_json_base_path = &catdir(CACHE_FMA_PATH,$mr_version,'common','*',$rep_ci_id,$rep_cb_id,$md_id,$mv_id,$mr_id,$t_type);
...
#								print STDERR __LINE__.":\$objfile=[$objfile]\n";
								&load_art_file_fromDB(art_id=>$art_id,hist_serial=>$art_hist_serial) unless(-e $objfile);
                   ...
                   where
                    ...
                    (md_id,mv_id,mr_id,cdi_id) in (
                        ...
                        from
                         representation
                        where
                         md_id=? and mv_id=? and mr_id<=? and bul_id=? and cdi_id in (%s)
                        group by
                         md_id,mv_id,cdi_id
                     )
               )
...
#=pod
							for(my $i=0;$i<360;$i+=5){
								&composite_bulk($i,$target_png_fmt,$larger_png_fmt,$angle_png_fmt,$larger_size);
								next;
...
=pod
							...
							for(my $i=0;$i<72;$i++){
								...
								push(@threads,threads->new(\&composite_bulk, $angle, $target_png_fmt, $larger_png_fmt, $angle_png_fmt, $larger_size));
							}
