...
use constant {
	...
	CHECK_USE_DATE_TIME => 0,	#日時で確認
	CHECK_EXISTS_FILE => 1,	#ファイルの有無で確認
...
#		push(@USE_CDI_NAMES, $_) for(split(/[^A-Za-z0-9]/,$cdi_name));
		push(@USE_CDI_NAMES, $_) for(split(/[,:;]+/,$cdi_name));
	}
}
&cgi_lib::common::message(\@USE_CDI_NAMES);
#exit;
...
if(scalar @MD_IDS == 0){
	my $sql = qq|select md_id from model where md_use group by md_id|;
	my $sth = $dbh->prepare($sql) or die $dbh->errstr;
	$sth->execute() or die $dbh->errstr;
	my $column_number = 0;
	my $md_id;
...
select EXTRACT(EPOCH FROM cm_modified) as cm_modified from concept_art_map_modified where (ci_id,cb_id,md_id,mv_id,mr_id,bul_id,cdi_id) in (
 ...
 group by
  ci_id,cb_id,md_id,mv_id,bul_id,cdi_id
				...
				my $sth_cd = $dbh->prepare(qq|
select EXTRACT(EPOCH FROM GREATEST(cd_entry,seg_entry)) as cd_entry,seg_thum_fgcolor from (
select
 cdi_id,
 cd_entry,
 seg_id
...
left join(select seg_id,seg_thum_fgcolor,seg_entry from concept_segment) as seg on seg.seg_id=cd.seg_id
where
 cdi_id=?
|) or die $dbh->errstr;
...
#					}
				...
				&cgi_lib::common::message(scalar @CDI_NAMES);
...
#				die __LINE__.":\n";
				if(defined $all_map_cdi_names && ref $all_map_cdi_names eq 'HASH'){
					&cgi_lib::common::message(scalar keys(%$all_map_cdi_names));
#					&cgi_lib::common::message(exists $all_map_cdi_names->{'FMA77732'} ? $all_map_cdi_names->{'FMA77732'} : undef);
				}
...
#					print STDERR __LINE__.":\$sql_cmm=[$sql_cmm]\n";
					...
					print STDERR __LINE__.":\$sth_cmm->execute()=[".$sth_cmm->rows()."]\n";
					if($sth_cmm->rows()>0){
						my $cdi_name;
						my $cm_modified;
						my $column_number = 0;
					...
					&cgi_lib::common::message(scalar keys(%$all_map_cdi_names));
					&cgi_lib::common::message(exists $all_map_cdi_names->{'FMA50255'} ? $all_map_cdi_names->{'FMA50255'} : 0);

					my $sth_cd = $dbh->prepare(qq|
select seg_thum_fgcolor from (
select
 cdi_id,
 seg_id
from
...
left join(select seg_id,seg_thum_fgcolor from concept_segment) as seg on seg.seg_id=cd.seg_id
where
 cdi_id=?
|) or die $dbh->errstr;
...
#						$rep_entry = $cm_modified if(defined $cm_modified && $rep_entry < $cm_modified);
						...
						my $rep_color = (ref  $all_map_cdi_names->{$cdi_name} eq 'HASH' ? $all_map_cdi_names->{$cdi_name}->{'seg_thum_fgcolor'} : undef);
						unless(defined $rep_color){
							my $cdi_id = exists $CDI_NAME2ID{$cdi_name} ? $CDI_NAME2ID{$cdi_name} : undef;
							if(defined $cdi_id){
								$sth_cd->execute($cdi_id) or die $dbh->errstr;
								if($sth_cd->rows()>0){
									my $seg_thum_fgcolor;
									$sth_cd->bind_col(1, \$seg_thum_fgcolor, undef);
									$sth_cd->fetch;
									$rep_color = $seg_thum_fgcolor if(defined $seg_thum_fgcolor);
								}
								$sth_cd->finish;
							}
						}
...
#							rep_color    => (ref  $all_map_cdi_names->{$cdi_name} eq 'HASH' ? $all_map_cdi_names->{$cdi_name}->{'seg_thum_fgcolor'} : undef),
						...
						if(defined $cdi_name && $cdi_name eq 'FMA77732'){
							&cgi_lib::common::message($all_map_cdi_names->{$cdi_name});
							&cgi_lib::common::message($CDI_NAMES[-1]);
						}
...
#				print __LINE__.":\$target_cdi_name=[".$target_cdi_name."]\n" if(defined $target_cdi_name);

				if(scalar @USE_CDI_NAMES){
					if(defined $target_cdi_name && $target_cdi_name eq 'FMA77732'){
						print __LINE__.":\$target_cdi_name=[".$target_cdi_name."][".(scalar grep {$_ eq $target_cdi_name} @USE_CDI_NAMES)."]\n";
						&cgi_lib::common::message($t_b_id);
					}
				...
				my $mca_entry;
				my $sth_mca;
				my $sql_rep = qq|select rep_id,md5(mca.art_ids_arr),rep.mca_id from representation AS rep left join (SELECT * FROM mapping_concepts_arts) AS mca ON mca.mca_id=rep.mca_id where rep_delcause is null and (md_id,mv_id,mr_id,bul_id,rep.cdi_id) in (select md_id,mv_id,max(mr_id),bul_id,cdi_id from representation where md_id=$md_id and mv_id=$mv_id and bul_id=$bul_id and cdi_id=? group by md_id,mv_id,bul_id,cdi_id)|;
				my $sth_rep = $dbh->prepare($sql_rep) or die $dbh->errstr;
				$sth_rep->execute($CDI_NAME2ID{$target_cdi_name}) or die $dbh->errstr;
				if($sth_rep->rows()>1){
					say STDERR __LINE__.':'.$sql_rep;
					say STDERR __LINE__.':'.$target_cdi_name;
					say STDERR __LINE__.':'.$CDI_NAME2ID{$target_cdi_name};
					die __LINE__.':'.'There are two or more rep_id ['.$sth_rep->rows().']';
					$sth_rep->finish;
					undef $sth_rep;
					next;
				}
				...
				$sth_rep->bind_col(++$column_number, \$mca_id, undef);
				$sth_rep->bind_col(++$column_number, \$mca_id_org, undef);
				$sth_rep->fetch;
				$sth_rep->finish;
				undef $sth_rep;
...
#					say STDERR __LINE__.':'.$sql_rep;
					say STDERR __LINE__.':'.$target_cdi_name;
					say STDERR __LINE__.':'.$CDI_NAME2ID{$target_cdi_name};
#					die __LINE__.':'.'Undefined rep_id';
					say STDERR __LINE__.':'.sprintf('Undefined rep_id [%s]',$target_cdi_name);
					next;
				}

				unless(defined $mca_id_org){
					say STDERR __LINE__.':'.'Undefined mca_id';
					next;
				}

				$sth_mca = $dbh->prepare(qq|select EXTRACT(EPOCH FROM mca_entry) from mapping_concepts_arts where mca_id=?|) or die $dbh->errstr;
				$sth_mca->execute($mca_id_org) or die $dbh->errstr;
				$column_number = 0;
				$sth_mca->bind_col(++$column_number, \$mca_entry, undef);
				$sth_mca->fetch;
				$sth_mca->finish;
				undef $sth_mca;
				if(defined $mca_entry){
					$rep_entry = $mca_entry;
				}else{
					die sprintf('Undefined mca_id[%d][%s]',__LINE__,$mca_id);
					say STDERR __LINE__.':'.'Undefined mca_id';
					next;
				}
...
##				my($img_prefix,$img_path) = &getCmImagePrefix($mr_version,undef,$target_cdi_name);
				...
				say STDERR __LINE__.':'.$rep_id;
				say STDERR __LINE__.':'.$mca_id;

				my ($cm_img_prefix,$cm_img_path) = &getCmImagePrefix($mr_version,$bul_id,$target_cdi_name);
				say STDERR __LINE__.':'.$cm_img_prefix;

				($img_prefix,$img_path) = &getMcaImagePrefix($mca_id,$rep_ci_id,$rep_cb_id);
				say STDERR __LINE__.':'.$img_prefix;
#				die 'DEBUG';
...
#				my $cache_json_base_path = sprintf(qq|/bp3d/cache_fma/%s/common|,$mr_version);
				...
				&File::Path::make_path($cm_img_path,{verbose => 0, mode => 0777}) unless(-e $cm_img_path);
...
#				print __LINE__.":\$txt_file=[",$txt_file,"][",(-e $txt_file ? 1 : 0),"][",(-s $txt_file),"]\n";
				...
				my $cm_file_info = &getImageFileList($cm_img_prefix);
...
#				if(defined $t_b_id->{'rep_delcause'} || scalar @USE_CDI_NAMES || $config->{'force'}){
				if(defined $t_b_id->{'rep_delcause'} || $config->{'force'}){
				...
				unless(defined $chk){
					foreach my $chk_file (@chk_files){
						...
						if(-e $chk_file && -s $chk_file && $chk_mtime>=floor($rep_entry)){
							if(CHECK_USE_DATE || CHECK_USE_DATE_TIME){
						...
						&cgi_lib::common::message($chk_file) if(CHECK_EXISTS_FILE);
...
&cgi_lib::common::message(sprintf('$chk=[%d]',defined $chk ? $chk : 0));
#die __LINE__;

				unless(defined $chk){
					if(defined $cm_img_prefix && defined $cm_file_info && ref $cm_file_info eq 'HASH'){
						my $target_files;
						push(@$target_files,$txt_file);
						push(@$target_files,@$imgsL);
						push(@$target_files,@$imgsM);
						push(@$target_files,@$imgsS);
						push(@$target_files,@$imgsXS);
						push(@$target_files,qq|$img_prefix.gif|);

						my $link_files;
						push(@$link_files,qq|$cm_img_prefix.txt|);
						push(@$link_files,@{$cm_file_info->{'imgsL'}});
						push(@$link_files,@{$cm_file_info->{'imgsM'}});
						push(@$link_files,@{$cm_file_info->{'imgsS'}});
						push(@$link_files,@{$cm_file_info->{'imgsXS'}});
						push(@$link_files,qq|$cm_img_prefix.gif|);

						if(scalar @$target_files == scalar @$link_files){
							&cgi_lib::common::message($cm_img_path);
							for(my $i=0;$i<scalar @$target_files;$i++){

								unless($config->{'force-link'}){
									if(-e $link_files->[$i] && -f $link_files->[$i] && -l $link_files->[$i] && -s $link_files->[$i]){
#										&cgi_lib::common::message(sprintf('%s [%6d]',$link_files->[$i],-s $link_files->[$i]));
										next;
									}
								}
#										&cgi_lib::common::message(sprintf('%s [%6d][%d][%d][%d]',$link_files->[$i],-s $link_files->[$i],-e $link_files->[$i] ? 1 : 0,-f $link_files->[$i] ? 1 : 0,-e $link_files->[$i] ? 1 : 0));

#								&cgi_lib::common::message($target_files->[$i]);
#								&cgi_lib::common::message(&abs2rel( $target_files->[$i], $cm_img_path ));
#								&cgi_lib::common::message($link_files->[$i]);
#die __LINE__;

								my $target_rep_path = &abs2rel( $target_files->[$i], $cm_img_path );
								unlink $link_files->[$i];# if(-e $link_files->[$i]);
								symlink $target_rep_path, $link_files->[$i];
							}
						}
					}
					next;
...
#									die __LINE__.":DEBUG\n";
							...
							$tmp_img_prefix = &catdir($tmp_base_path,&File::Basename::basename($img_prefix));
...
#die __LINE__.":[",$?,"]\n";
							...
							unlink qq|$img_prefix.gif|;# if(-e qq|$img_prefix.gif|);
							symlink &File::Basename::basename($img_prefix).qq|_$sizeStrM.gif|,qq|$img_prefix.gif| if(-e $imgsM->[0]);
							if(CHECK_USE_DATE || CHECK_USE_DATE_TIME){
