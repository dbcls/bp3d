...
#	my @FILES;
	...
	my @FILES = ();
	push(@FILES,@$obj_files1) if(defined $obj_files1 && ref $obj_files1 eq 'ARRAY');
	push(@FILES,@$obj_files2) if(defined $obj_files2 && ref $obj_files2 eq 'ARRAY');
	push(@FILES,@$obj_files3) if(defined $obj_files3 && ref $obj_files3 eq 'ARRAY');
	if(scalar @FILES){
...
#	'partof' => 'FMA20394',
	...
	'fma' => [qw/FMA7163 FMA5018 FMA242789/],
};

#my $version = qq|3.0|;
#my $tree = qq|bp3d|;
my $copyrightL = &catfile($FindBin::Bin,'img','copyrightM.png');
my $copyrightS = &catfile($FindBin::Bin,'img','copyrightS.png');
...
#	}
	map { push(@USE_CDI_NAMES, split(/[^A-Za-z0-9\-]/,$_)) } @{$config->{'target'}};
}
...
|;
	...
	foreach my $version (@VERSION){
		...
		unless(exists $config->{'target'} && defined $config->{'target'} && ref $config->{'target'} eq 'ARRAY' && scalar @{$config->{'target'}}){
			my $sql = qq|
select
 cdi_id
from
...
group by
 cdi_id
|;
			my $sth = $dbh->prepare(qq|select cdi_name from concept_data_info where cdi_delcause is null and ci_id=$ci_id and cdi_id IN ($sql)|) or die $dbh->errstr;
			$sth->execute() or die $dbh->errstr;
			my $column_number = 0;
			my $cdi_name;
			$sth->bind_col(++$column_number, \$cdi_name, undef);
			while($sth->fetch){
				push(@USE_CDI_NAMES, $cdi_name);
			}
			$sth->finish;
			undef $sth;
		}
...
#			);
			...
			print __LINE__.":CALL BITS::ConceptArtMapModified::exec()\n";
...
#			die __LINE__.":\n";
		...
		print __LINE__.":CALL get_all_colors()\n";
		...
		print __LINE__.":CALL get_art_file_bbox()\n";
...
SQL
		...
		print __LINE__.":CALL tbp_max_enter()\n";
		my $tbp_max_enter;
...
#							my $sth_cm = $dbh->prepare($sql_cm) or die $dbh->errstr;
							...
							my($art_files,$art_files2) = &BITS::ArtFile::get_art_file(dbh=>$dbh,md_id=>$md_id,mv_id=>$mv_id,mr_id=>$mr_id,ci_id=>$ci_id,cb_id=>$cb_id,bul_id=>$bul_id,cdi_name=>$target_cdi_name);
							if(defined $art_files && ref $art_files eq 'ARRAY'){
								print __LINE__.":\t\$art_files=[".&JSON::XS::encode_json($art_files)."]\n";
...
#										hist_serial=>$art_hist_serial,
							...
							if(defined $art_files2 && ref $art_files2 eq 'ARRAY'){
								print __LINE__.":\t\$art_files2=[".&JSON::XS::encode_json($art_files2)."]\n";
#die __LINE__;
								foreach my $art_file (@$art_files2){
									my $art_id = $art_file->{'art_id'};
									my $art_ext = $art_file->{'art_ext'};
									my $art_hist_serial = $art_file->{'hist_serial'};
...
#								exit(0);
							...
							print __LINE__.":obj_files1=[",(scalar @$obj_files1),"]\n" if(defined $obj_files1 && ref $obj_files1 eq 'ARRAY');
							print __LINE__.":obj_files3=[",(scalar @$obj_files3),"]\n" if(defined $obj_files3 && ref $obj_files3 eq 'ARRAY');
#							exit 1;
...
#										hist_serial=>$art_hist_serial,
							...
							my $bp3d_objs = &load_art_file(md_id=>$md_id,mv_id=>$mv_id,mr_id=>$mr_id,obj_files1=>$obj_files1,obj_files2=>$obj_files2,obj_files3=>$obj_files3);
							...
							my $largerbbox = [];
							my $bound_obj_files;
							push(@{$bound_obj_files}, @{$obj_files1});
							push(@{$bound_obj_files}, @{$obj_files3}) if(defined $obj_files3 && ref $obj_files3 eq 'ARRAY');
							my $b1 = $obj2image->bound($bound_obj_files);
							$_+=0 for(@$b1);
...
#								print __LINE__.":\t\$obj_files2=[".&JSON::XS::encode_json($obj_files2)."]\n";
								print __LINE__.":\t\$obj_files3=[".&JSON::XS::encode_json($obj_files3)."]\n";
								...
								$bb = $obj2image->obj2animgif($sizeL,$obj_files1,$obj_files2,$obj_files3,$tmp_dest_prefix,$b1YRange,$largerbbox,$largerbboxYRange);
								undef $bb;
								undef $obj2image;
								undef $bp3d_objs;
								undef $obj_files1;
