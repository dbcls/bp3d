...
sub resize {
	...
	if(defined $in_file && defined $geometry && defined $img_size){
		if($geometry ne $img_size){
			my $cmd;
			if(defined $out_file){
				$cmd = File::Spec->catfile(IMAGEMAGICK_PATH,qq|convert|);
			}else{
				$cmd = File::Spec->catfile(IMAGEMAGICK_PATH,qq|mogrify|);
			}
...
#		system(IMAGEMAGICK_PATH.qq|composite -gravity southeast $larger_file $target_file $out_file|);
		...
		my $cmd = File::Spec->catfile(IMAGEMAGICK_PATH,qq|composite|);
		my %C_OPT = (
			'-gravity' => 'southeast'
		);
...
sub composite_bulk {
	...
	if(defined $target_file && defined $larger_file && defined $out_file && -e $target_file && -s $target_file && -e $larger_file && -s $larger_file){
		...
		my @CMD = ();
		my $composite = File::Spec->catfile(IMAGEMAGICK_PATH,qq|composite|);
		my $convert = File::Spec->catfile(IMAGEMAGICK_PATH,qq|convert|);
		my $mogrify = File::Spec->catfile(IMAGEMAGICK_PATH,qq|mogrify|);
...
sub animatedGIF {
	...
	if(defined $in_files && ref $in_files eq 'ARRAY' && defined $out_file){
		my $cmd = File::Spec->catfile(IMAGEMAGICK_PATH,qq|convert|);
		my %OPT = (
			'-dispose' => 'Background',
			'-delay' => 0,
			'-loop' => 0,
...
sub get_tmppath {
	my $lock_no = shift;
	if(defined $lock_no){
		return File::Spec->catdir(TMPFS,$lock_no);
	}else{
