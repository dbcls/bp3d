...
#	my $header = qq|#ID	Name	Name(L)	Name(J)	Name(K)	Synonym	Synonym(J)	Definition	Definition(J)	TAID	Tree|;
			...
			&cgi_lib::common::message(&cgi_lib::common::encodeJSON($header,1),$LOG) if(defined $LOG && defined $header);
...
#				my $concept_art_map_part = $D->{'concept_art_map_part'};
				...
				foreach my $data (@{$D->{'concept_art_map_part'}}){
					...
					if(index($data,'#') == 0){
						undef $header;
						$header = &get_header($data);
						undef $header unless(defined $header && exists $header->{$KEYS{'cmp_title'}} && exists $header->{$KEYS{'cmp_abbr'}} && exists $header->{$KEYS{'cmp_prefix'}} && exists $header->{$KEYS{'bul_name_e'}});
						&cgi_lib::common::message(&cgi_lib::common::encodeJSON($header,1),$LOG) if(defined $LOG && defined $header)
					}elsif(defined $header){
						my $hash = &set_data2hash($header,$data);
						unless(defined $hash && exists $hash->{$KEYS{'cmp_title'}} && defined $hash->{$KEYS{'cmp_title'}} && length $hash->{$KEYS{'cmp_title'}}){
							undef $hash;
							next;
						}
	...
	unless(defined $concept_art_map_part && ref $concept_art_map_part eq 'ARRAY' && scalar @$concept_art_map_part){
		...
		$concept_art_map_part->[1]->{$KEYS{'bul_name_e'}} = 'is_a';
		...
		$concept_art_map_part->[2]->{$KEYS{'bul_name_e'}} = 'is_a';
		...
		$concept_art_map_part->[3]->{$KEYS{'bul_name_e'}} = 'part_of';
		...
		$concept_art_map_part->[4]->{$KEYS{'bul_name_e'}} = 'is_a';
		...
		$concept_art_map_part->[5]->{$KEYS{'bul_name_e'}} = 'is_a';
		...
		$concept_art_map_part->[6]->{$KEYS{'bul_name_e'}} = 'is_a';
	}
...
#		}
		foreach my $p (@$concept_art_map_part){
			...
			my $bul_name_e = $p->{$KEYS{'bul_name_e'}};
...
&cgi_lib::common::message('登録数:'.$exec_ins,$LOG) if(defined $LOG);
	...
	foreach my $hash_art_id (keys %art_id_2_hash){
		...
		if(exists $header->{$KEYS{'cmp_title'}} && defined $header->{$KEYS{'cmp_title'}}){
...
#					$cmp_abbr = $2 || '';
#				}
				$cdi_name = $hash->{$KEYS{'cdi_name'}};
				$cmp_title = $hash->{$KEYS{'cmp_title'}};
				$cmp_abbr = '';
...
#				}
	...
	&cgi_lib::common::message(&cgi_lib::common::encodeJSON(\%MAPPED_ART_ID,1),$LOG) if(defined $LOG);
