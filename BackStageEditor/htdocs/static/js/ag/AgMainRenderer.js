	...
	function onObjectMouseMove(event){
		...
		tipTask.delay(250,function(event){
			...
			if(Ext.isEmpty(intersects)){
				...
				var text = '';
				if(!Ext.isEmpty(intersects[0].object.record.data.version)) text += '<h3>['+intersects[0].object.record.data.version+']</h3>';
				if(!Ext.isEmpty(intersects[0].object.record.data.b_id)) text += '<h3>['+intersects[0].object.record.data.b_id+']</h3>';
				if(!Ext.isEmpty(intersects[0].object.record.data.f_id)) text += '<h3>['+intersects[0].object.record.data.f_id+']</h3>';
				if(!Ext.isEmpty(intersects[0].object.record.data.art_id)) text += '<h3>['+intersects[0].object.record.data.art_id+']</h3>';
				if(!Ext.isEmpty(intersects[0].object.record.data.group)) text += '<h3>['+intersects[0].object.record.data.group+']</h3>';
				text += intersects[0].object.record.data.name;
	/*
					...
					if(!Ext.isEmpty(intersects) && intersects[0].distance<Math.abs(camera.far-camera.near)){
						...
						if(record){
							...
							function makePickRecord(record){
								...
								return Ext.create('PALLETPARTS',param);
							}
	...
	function initINTERSECTED(){
		if(INTERSECTED){
			if(INTERSECTED.object.record){
				var color = INTERSECTED.object.record.data.color;
...
//							pinObj.add( line );
	...
	*/
	...
	var BP3D_C2P_TREE;
	/*
	var BP3D_C2P_TREE_params = {
		node : 'c2p',
		ci_id : DEF_VALUES.ci_id,
		cb_id : DEF_VALUES.cb_id
	};
	if(Ext.isEmpty(window.DEF_VERSION_DATAS)){
		BP3D_C2P_TREE_params.model = DEF_MODEL;
		BP3D_C2P_TREE_params.version = DEF_VERSION;
		BP3D_C2P_TREE_params.ag_data = DEF_AG_DATA;
		BP3D_C2P_TREE_params.tree = DEF_TREE;
	}else{
		BP3D_C2P_TREE_params.version_datas = Ext.JSON.encode(DEF_VERSION_DATAS.datas);
	}
	Ext.Ajax.request({
		url     : 'get-bp3d-tree.cgi',
		params  : BP3D_C2P_TREE_params,
		timeout : 300000,
		success : function(conn,response,options){
			try{
				...
				if(json){
	//				console.log("BP3D_C2P_TREE:LOAD:OK!!");
					BP3D_C2P_TREE = json;
	//				console.log(BP3D_C2P_TREE);
					bp3d_parts_store.load();
				}else{
	//				console.log("BP3D_C2P_TREE:LOAD:ERR!!");
				}
			}catch(e){
				console.log("BP3D_C2P_TREE:LOAD:ERR!!:["+e+"]");
			}
		},
		failure : function(conn,response,options){
			console.log("BP3D_C2P_TREE:LOAD:NG!!");
		}
	});
	*/
	...
	//				console.log("extension_parts_store.update():["+operation+"]");
					...
					if(self.isEmptyMesh(path)){
						if(record.data.selected) self.loadBp3dParts.task.delay(250);
					}else{
	...
	var camera_focus = function (records,selected){
		if(Ext.isEmpty(records)) records = bp3d_parts_store.getBufferRange();
		if(Ext.isEmpty(selected)) selected = false;
		if(records.length>0){

			var c = camera_center(records,selected);
			if(Ext.isEmpty(c)) return;

	//			setCameraAndTarget(camera.position,target,camera.up);
	//			camera.position.copy(m_ag_cameraPos);
	//			camera.up.copy(m_ag_upVec);
	//			target.copy(m_ag_targetPos);

//			var yrange = Math.max(c.xmax-c.xmin,c.zmax-c.zmin);
			var valArr = [c.xmax-c.xmin,c.ymax-c.ymin,c.zmax-c.zmin];
			valArr.push(Math.pow(valArr[0]*valArr[0] + valArr[1]*valArr[1] + valArr[2]*valArr[2], 0.5));
			var yrange = Ext.Array.max(valArr);
	...
	var camera_center = function (records,selected){
		if(Ext.isEmpty(records)) records = bp3d_parts_store.getBufferRange();
		if(Ext.isEmpty(selected)) selected = false;
		var xmax,xmin,xcenter;
		var ymax,ymin,ycenter;
		var zmax,zmin,zcenter;
		for(var i=0;i<records.length;i++){
			var data = records[i].data;
			if(data.c_num>0) continue;
			if(selected && (!data.selected || !data.focused) && self.getMesh(data.b_id)) continue;

			try{
				if(self.getMesh(data.b_id)){
					if(!self.getMesh(data.b_id).geometry.boundingBox) self.getMesh(data.b_id).geometry.computeBoundingBox();
					var b = self.getMesh(data.b_id).geometry.boundingBox;
					if(Ext.isEmpty(xmax) || xmax < b.max.x) xmax = b.max.x;
					if(Ext.isEmpty(xmin) || xmin > b.min.x) xmin = b.min.x;
					if(Ext.isEmpty(ymax) || ymax < b.max.y) ymax = b.max.y;
					if(Ext.isEmpty(ymin) || ymin > b.min.y) ymin = b.min.y;
					if(Ext.isEmpty(zmax) || zmax < b.max.z) zmax = b.max.z;
					if(Ext.isEmpty(zmin) || zmin > b.min.z) zmin = b.min.z;
				}
			}catch(e){
				console.log(e);
			}
	...
	/*
			if(Ext.isEmpty(xmax) || xmax < data.xmax) xmax = data.xmax;
			if(Ext.isEmpty(xmin) || xmin > data.xmin) xmin = data.xmin;
			if(Ext.isEmpty(ymax) || ymax < data.ymax) ymax = data.ymax;
			if(Ext.isEmpty(ymin) || ymin > data.ymin) ymin = data.ymin;
			if(Ext.isEmpty(zmax) || zmax < data.zmax) zmax = data.zmax;
			if(Ext.isEmpty(zmin) || zmin > data.zmin) zmin = data.zmin;
	*/

		}

		if(
			Ext.isEmpty(xmax) || Ext.isEmpty(xmin) ||
			Ext.isEmpty(ymax) || Ext.isEmpty(ymin) ||
			Ext.isEmpty(zmax) || Ext.isEmpty(zmin)
		){
			return undefined;
		}else{
	//		console.log("camera_center(1):["+target.x+"]["+target.y+"]["+target.z+"]");
			xcenter = target.x = xmax - ((xmax - xmin) / 2);
			ycenter = target.y = ymax - ((ymax - ymin) / 2);
			zcenter = target.z = zmax - ((zmax - zmin) / 2);
	//		console.log("camera_center(2):["+target.x+"]["+target.y+"]["+target.z+"]");

			var longitude = m_ag_longitude;
			var latitude = m_ag_latitude;
			setAutoCameraPos({
				xmax : xmax,
				xmin : xmin,
				ymax : ymax,
	...
	//			setCameraAndTarget(camera.position,target,camera.up);
			...
			camera.up.copy(m_ag_upVec);
			target.copy(m_ag_targetPos);
	//		console.log("camera_center(3):["+target.x+"]["+target.y+"]["+target.z+"]");
		}
		return {
			xmax    : xmax,
			xmin    : xmin,
	...
	//					console.log("reader.exception()");
			...
			listeners : {
				metachange : function(){
	//				console.log("metachange.exception()");
				}
			}
		},
		listeners : {
			add : function(){
	//			console.log("bp3d_parts_store.add()");
			},
			beforeprefetch : function(){
	//			console.log("bp3d_parts_store.beforeprefetch()");
			},
			beforesync : function(){
	//			console.log("bp3d_parts_store.beforesync()");
			},
			clear : function(){
	//			console.log("bp3d_parts_store.clear()");
			},
			datachanged : function(){
	//			console.log("bp3d_parts_store.datachanged():["+bp3d_parts_store.getTotalCount()+"]");
			},
			groupchange : function(){
	//			console.log("bp3d_parts_store.groupchange()");
			},
			metachange : function(){
	//			console.log("bp3d_parts_store.metachange()");
			},
			prefetch : function(){
	//			console.log("bp3d_parts_store.prefetch()");
			},
			refresh : function(){
	//			console.log("bp3d_parts_store.refresh():["+bp3d_parts_store.getTotalCount()+"]");
			},
			remove : function(){
	//			console.log("bp3d_parts_store.remove()");
			},
			write : function(){
	//			console.log("bp3d_parts_store.write()");
			},

			beforeload : function(store,operation,eOpts){
	//			console.log("bp3d_parts_store.beforeload()");
	...
	//			console.log("bp3d_parts_store.load():["+(records?records.length:0)+"]");
				store.isLoaded = true;
			...
			update  : function(store,record,operation){
	//			console.log("bp3d_parts_store.update()");
				if(operation==Ext.data.Record.EDIT){
				}else if(operation==Ext.data.Record.COMMIT){
	//				console.log("bp3d_parts_store.update():["+record.data.name+"]["+record.data.c_num+"]");
	...
	//							console.log("bp3d_parts_store.update():["+r.data.b_id+"]["+i+"]["+modifiedFieldNames.join(",")+"]");

							}else{
								r.cancelEdit();

	//							console.log("bp3d_parts_store.update():["+r.data.b_id+"]["+i+"]["+modifiedFieldNames.length+"]");
							}
...
*/
	...
	*/
	...
	/**/
		if(bp3d_parts_store.sorters.getAt(0).property != 'c_num_'+p_tree){
	...
	//console.log("createLocaleHashStr():jsonObj.Part=["+Ext.isEmpty(jsonObj.Part)+"]");

	//	console.log("createLocaleHashStr():bp3d_parts_store");
...
//					if(Ext.isEmpty(use_paths[path])) loadBp3dParts.hiddenMesh(path2meshs[path]);
	...
	//							var record = extension_parts_store.findRecord('path',path,0,false,true,true);
								var idx = extension_parts_store.findBy(function(r,id){
									if(r.data.grouppath==PartGroupPath && r.data.path==path) return true;
									return false;
								});
								var record = extension_parts_store.getAt(idx);
								if(!record){
									record = Ext.create('EXTENSIONPARTS',{});
									isAdd = true;
								}
								...
								if(record.get('group') != PartGroupName){
									record.set('group',PartGroupName);
									modifiedFieldNames.push('group');
								}
								if(record.get('grouppath') != PartGroupPath){
									record.set('grouppath',PartGroupPath);
	...
	//				console.log("createLocaleHashStr():full_path=["+getRecordPath(record)+"]");
	...
	self.getRecordPath = function(record){
		var path = '';
		if(!Ext.isEmpty(record.data.grouppath)) path = record.data.grouppath+'/';
...
//						me.loadMask = new Ext.LoadMask(me, config);
	...
	//					console.log("trigger('hideLoading')");
		...
		hideLoading : function(){
			self.loadBp3dParts.hideLoadingTask.cancel();
			if(!bp3d_parts_store.isLoaded || bp3d_parts_store.isLoading()){//初期化時
				self.loadBp3dParts.hideLoadingTask.delay(250);
			}else{
				self.loadBp3dParts.setLoading(false);
			}
...
//				this.renderDepth = this.material.opacity;
	...
	//		mesh.visible = record.data.remove ? false : record.data.selected;
			setMeshVisible(mesh.name,record.data.remove ? false : record.data.selected);
			if(!mesh.visible) return;

			if(mesh.visible){
				var color;
				if(colorHeatMap.isVisibled() && record.data.scalar_flag && Ext.isNumber(record.data.scalar)){
					color = colorHeatMap.calcColorValue(record.data.scalar);
				}else{
...
//				mesh.material.ambient.setHex( color );
				...
				mesh.material.opacity = record.data.opacity;
				mesh.material.wireframe = Ext.isEmpty(record.data.representation) ? false : record.data.representation == 'wireframe' ? true : false;
	...
	//		mesh2.material.side = THREE.BackSide;
			...
			if(Ext.isBoolean(record.data.focused)){
				if(record.data.focused){
		...
		exec : function(){
			...
			while(!Ext.isEmpty(records)){
				...
				if(record.data.selected){
					if(!Ext.isEmpty(record.data.c_num) && record.data.c_num>0) continue;
					if(Ext.isEmpty(path)) continue;

					if(!self.isEmptyMesh(path)){
//					path2meshs[path].visible = record.data.remove ? false : record.data.selected;
						setMeshVisible(path,record.data.remove ? false : record.data.selected);
					...
					if(record.data.remove){
	//					console.log("loadBp3dParts.exec():["+record.data.name+"]["+record.data.remove+"]");
	//					self.loadBp3dParts._cnt++;
	//					self.loadBp3dParts.unmask();
	//					self.loadBp3dParts.exec();
						continue;
	...
	//				self.loadBp3dParts.mask((record.data.b_id ? record.data.b_id : record.data.name) +  (Ext.isEmpty(record.data.filesize) || record.data.filesize == 0 ? '' : ' ['+ Ext.util.Format.fileSize(record.data.filesize) + ']'));

					var name = record.data.name;
					if(record.data.f_id && record.data.art_id){
						name = record.data.cdi_name + ' - ' + record.data.art_id;
					}else if(record.data.cdi_name){
						name = record.data.cdi_name;
					}else if(record.data.art_id){
						name = record.data.art_id;
					}else if(record.data.rep_id){
						name = record.data.rep_id;
					}
					self.loadBp3dParts.mask(name +  (Ext.isEmpty(record.data.filesize) || record.data.filesize == 0 ? '' : ' ['+ Ext.util.Format.fileSize(record.data.filesize) + ']'));

					var url = path;
					try{
						url+='?'+Math.floor(record.data.modified.getTime()/1000);
					}catch(e){
						...
						jsonLoader.load(url,function(success,geometry){
							if(success){
								var mesh = self.loadBp3dParts.createMesh({
									...
									color    : color,
									opacity  : record.data.opacity,
									visible  : record.data.remove ? false : record.data.selected,
									representation : record.data.representation
								});
								mesh.name = path;
...
//					var yrange = Math.max(size.x,size.y,size.z,Math.pow(size.x*size.x + size.y*size.y + size.z*size.z, 0.46));
	...
	//}
				...
				Ext.each(bp3d_parts_store.getRange(),function(record,i,a){
					if(!Ext.isBoolean(record.data.focused)) return true;
					record.beginEdit();
					record.set('focused',null);
					record.commit(false);
					record.endEdit(false,['focused']);
	...
	//			console.log(record.data.name);
			...
			self.loadBp3dParts._num = bp3d_parts_store.getRange().length;
	...
	//		var range = bp3d_parts_store.getRange();
	//		console.log("loadBp3dPartsTask():bp3d_parts_store=["+range.length+"]");
	//		if(range.length>0){
	//			console.log(range);
	//		}

	//		console.log("loadBp3dPartsTask():extension_parts_store=["+extension_parts_store.getRange().length+"]");
			...
			self.loadBp3dParts._records = bp3d_parts_store.getRange().concat(extension_parts_store.getRange());
			self.loadBp3dParts.exec();
	...
	var createLocaleHashStrTask = new Ext.util.DelayedTask(function(){
		if(bp3d_parts_store.isLoading()) return;
...
//					}
					...
					if(record.get('filesize') != e.target._file.size){
						record.set('filesize',e.target._file.size);
						modifiedFieldNames.push('filesize');
					}
					if(record.get('mtime') != e.target._file.lastModifiedDate){
						record.set('mtime',e.target._file.lastModifiedDate);
						modifiedFieldNames.push('mtime');
					}

					mesh.geometry.computeBoundingBox();
					var bb = mesh.geometry.boundingBox;

					if(record.get('xmin') != bb.min.x){
						record.set('xmin',bb.min.x);
						modifiedFieldNames.push('xmin');
					}
					if(record.get('ymin') != bb.min.y){
						record.set('ymin',bb.min.y);
						modifiedFieldNames.push('ymin');
					}
					if(record.get('zmin') != bb.min.z){
						record.set('zmin',bb.min.z);
						modifiedFieldNames.push('zmin');
					}

					if(record.get('xmax') != bb.max.x){
						record.set('xmax',bb.max.x);
						modifiedFieldNames.push('xmax');
					}
					if(record.get('ymax') != bb.max.y){
						record.set('ymax',bb.max.y);
						modifiedFieldNames.push('ymax');
					}
					if(record.get('zmax') != bb.max.z){
						record.set('zmax',bb.max.z);
						modifiedFieldNames.push('zmax');
					}

					var center = new THREE.Vector3();
					center.addVectors( bb.max, bb.min );
					center.multiplyScalar( 0.5 );
					if(record.get('xcenter') != center.x){
						record.set('xcenter',center.x);
						modifiedFieldNames.push('xcenter');
					}
					if(record.get('ycenter') != center.y){
						record.set('ycenter',center.y);
						modifiedFieldNames.push('ycenter');
					}
					if(record.get('zcenter') != center.z){
						record.set('zcenter',center.z);
						modifiedFieldNames.push('zcenter');
					}
