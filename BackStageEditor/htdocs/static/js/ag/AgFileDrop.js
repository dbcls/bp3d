...
window.AgApp.prototype.fileDrop = function(event, eOpt){
	...
	try{
		...
		if(dataTransfer){
			...
			if(false && dataTransfer.items && dataTransfer.items.length>0){
...
//							msg: e.target.error.name + ' : ' + e.target.error.message,
//							buttons: Ext.Msg.OK,
//							icon: Ext.Msg.ERROR
//						});
...
window.AgApp.prototype._fileDropEntry = function(entry){
	var self = this;
	if(entry.isFile){
//		console.log(entry);
//		self.__dropfile_count++;
