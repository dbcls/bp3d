...
//							var artg_name = r.get('artg_name');
							var artg_name = null;
							var art_id = r.get('art_id');
							var cdi_name = r.get('cdi_name');
							var cm_use = r.get('cm_use');
							if(Ext.isEmpty(artg_name)){
							...
							else if(Ext.isEmpty(hash[artg_name])){
								hash[artg_name] = artg_name;
								artg_names.push(artg_name);
							}
							if(!cm_use && !Ext.isEmpty(cdi_name) && !Ext.isEmpty(art_id)){
								var cm_use_key = cdi_name+"\t"+art_id;
								if(Ext.isEmpty(cm_use_key_hash[cm_use_key])){
									cdi_name_hash[cm_use_key] = cm_use_key;
									cm_use_keys.push({
										cdi_name: cdi_name,
										art_id:   art_id,
										selected: r.get('selected'),
						...
						if(art_ids.length || cdi_names.length || cm_use_keys.length){
//							var params = app.getExtraParams() || {};
							var params = {};
							if(art_ids.length){
...
/*
							Ext.Ajax.request({
								...
								method: 'POST',
								params: Ext.apply({},app.getExtraParams(),params),
								success: function(response, options){
									...
									if(!Ext.isEmpty(options.params.art_ids) || !Ext.isEmpty(options.params.cdi_names) || !Ext.isEmpty(options.params.cm_use_keys)){
										AgPalletUtils.pasteListStore.suspendEvents(true);
//										if(addrecs.length) AgPalletUtils.pasteListStore.remove(addrecs);
										addrecs = [];

										Ext.iterate(json.datas,function(data){
											var modifiedFieldNames = [];
											var n_record = Ext.create(AgPalletUtils.pasteListStore.model.modelName,data);
											n_record.beginEdit();
										...
										if(addrecs.length) AgPalletUtils.pasteListStore.add(addrecs);
										AgPalletUtils.pasteListStore.resumeEvents();
									}
...
//									if(AgPalletUtils.pasteListCB) AgPalletUtils.pasteListCB(addrecs);
									...
									var selected_art_ids = app.getSelectedArtIds(AgPalletUtils.pasteListStore);
									var uploadObjectStore = Ext.data.StoreManager.lookup('uploadObjectStore');
									var sorters = app.getSelectedSorters();
...
*/
							...
							try{
								Ext.each(artg_names,function(artg_name,i,a){
									var r = uploadGroupStore.findRecord('artg_name',artg_name,0,false,true,true);
									if(r && !r.get('selected')){
										r.beginEdit();
										r.set('selected',true);
										r.endEdit(false,['selected']);
							...
							if(artg_names_selected.length){
								Ext.data.StoreManager.lookup('uploadObjectAllStore').on({
									load: function(store,records,successful,eOpts){
