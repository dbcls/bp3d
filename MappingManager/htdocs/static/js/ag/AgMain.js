...
//			console.log(recs);
	...
	$(document).on('dragstart','*[draggable="true"]',function(e){
		...
		e.stopPropagation();
//		var gridpanel = Ext.getCmp('upload-object-grid');
//		var gridview = gridpanel ? gridpanel.getView() : null;
		var viewid = Ext.get(node).getAttribute('data-boundview');
//		console.log(viewid);
		var gridview = viewid ? Ext.getCmp(viewid) : null;
		var record = gridview ? gridview.getRecord(node) : null;
		if(record){
			e.originalEvent.dataTransfer.setData('text/plain', Ext.encode(Ext.Object.merge({'data-boundview':viewid},record.getData())));
		}
//		return false;
	});
};
...
//										treecombo.setValue(record.get('artf_id'));
	...
	//														console.log(folders);
															...
															Ext.each(files,function(file){
																records.push([file.name,file.size,Ext.Date.parse(file.lastModified,'time'),file]);
																file_size += file.size;
															});
															...
															if(files.length>1){
																...
																fieldset.getComponent('file_last').setValue(Ext.util.Format.date(Ext.Date.parse(file.lastModified,'time'),self.FORMAT_DATE_TIME)).show();
											...
											listeners: {
												afterrender: function(field,eOpts){
													if(field.fileInputEl){
														field.fileInputEl.set({'multiple':'multiple','accept':'.obj,.tar,.tar.gz,.tgz,.gz,.Z,.zip,.bz2,.tar.bz2,.tbz,.lzma,.xz,.tar.xz,.txz'});
													}
												},
...
//						metaData.style = 'background:'+record.data.seg_color+';';
		...
		viewConfig: {
			...
			allowCopy: true,
			plugins: [{
				pluginId: 'upload-object-gridviewdragdrop',
        ...
        '<tr role="row" {[values.rowId ? ("id=\\"" + values.rowId + "\\"") : ""]} ',
            ...
            '{rowAttr:attributes} tabIndex="-1" draggable="{[ window.isAgDraggable ]}">',
            '<tpl for="columns">' +
                '{%',
        ...
        '</tr>',
			...
			listeners: {
				itemadd: function( records, index, node, eOpts ){
					console.log('itemadd', records, index, node, eOpts );
				},
				itemupdate: function( record, index, node, eOpts ){
					console.log('itemupdate', record, index, node, eOpts );
				}
			}
		},
		selType: 'rowmodel',
...
//						console.log(nodes);
			...
			'-',' ','-',{
				...
				xtype: 'button',
				tooltip: 'Draggable',
				iconCls: 'draggable-btn',
				enableToggle: true,
				pressed: window.isAgDraggable,
				listeners: {
					toggle: function(b,pressed){
						var gridpanel = b.up('gridpanel');
						var gridview = gridpanel ? gridpanel.getView() : null;
						var plugin = gridview ? gridview.getPlugin('upload-object-gridviewdragdrop') : null;
						if(plugin){
							if(pressed){
								plugin.disable();
							}
							...
							window.isAgDraggable = b.pressed;

							Ext.Array.each(gridview.getNodes(), function(node){
								Ext.get(node).set({draggable:b.pressed},true);
							});
						}
					}
				}
...
/*
				...
				gridpanel.getStore().on({
					load: function( store, records, successful, eOpts ){
						console.log('load', store, records, successful, eOpts );
					}
				});
*/
			},
...
//				autoScroll: true,
				columns: [
					...
					{text: AgLang.cmp_abbr,   dataIndex: 'cmp_id',      width:40, minWidth:40, hidden:false, hideable:false, draggable: false, xtype: 'agcolumnconceptartmappart'},
...
//												console.log('load:uxiframe():'+uxiframe.id);
										...
										columns: [
											...
											{text: AgLang.art_id,     dataIndex: 'art_id',       stateId: 'art_id',       width:60, minWidth:60, hidden:false, hideable:false, sortable:false, xtype:'agcolumn'},
				...
				listeners: {
					change: function(field,value,eOpts){
						...
						if(field.fileInputEl.dom.files && field.fileInputEl.dom.files.length){
							...
							formPanel.getComponent('file_last').setValue(Ext.util.Format.date(Ext.Date.parse(file.lastModified,'time'),self.FORMAT_DATE_TIME));
							...
							Ext.Msg.show({
								...
								fn: function(buttonId){
									...
									if(file_field && file_field.fileInputEl && file_field.fileInputEl.dom.files && file_field.fileInputEl.dom.files.length){
										...
										fd.append('last',file.lastModified);
