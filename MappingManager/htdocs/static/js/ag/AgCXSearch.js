...
window.AgApp.prototype.getCXSearchButton = function(){
	var self = this;
	return {
		itemId: 'cx_search',

		xtype: 'button',
		tooltip: AgLang.cx_search,
		text: 'CX',
		iconCls: 'cx-search-btn',
		disabled: false,
		listeners: {
			click: function(b){
				self.openCXSearchMngWin({
					title: b.tooltip,
					iconCls: b.iconCls,
					animateTarget: b.el
				});
...
window.AgApp.prototype.openCXSearchMngWin = function(aOpt){
	var self = this;
	aOpt = Ext.apply({},aOpt||{},{
		iconCls: 'cx-search-btn',
		title: AgLang.cx_search,
	});
...
//		{text: 'CXobj',       dataIndex: 'cx_group_id',       stateId: 'cx_group_id',        width: 54, minWidth: 54, hidden: false, hideable: true},
...
//					{text: AgLang.upload_modified, dataIndex: 'art_upload_modified',stateId: 'art_upload_modified', width: 112,  hidden: true, hideable: true, xtype: 'datecolumn', format: self.FORMAT_DATE_TIME },
	...
	if(Ext.isEmpty(self.__CXSearchMngWin)){

		var searchObjDStore = Ext.create('Ext.data.Store', {
			model: 'CX_PARTS',
			groupField: 'cx_group_id',
			remoteSort: false,
			remoteGroup: false,
			sorters: [{
				property: 'cx_group_id',
				direction: 'ASC'
			},{
				property: 'art_id',
				direction: 'ASC'
...
*/
		...
		var searchObjRecord = function(){
			var search_gridpanel = self.__CXSearchMngWin.down('gridpanel#search-gridpanel');
		...
		self.__CXSearchMngWin = Ext.create('Ext.window.Window', {
			title: aOpt.title,
			...
			items: {
				...
				items: [{
					margin: '5 5 0 5',
					emptyText: AgLang.cx_search_empty_text,
					fieldLabel: AgLang.cx_search,
					labelWidth: 30,
					hideLabel: true,
					xtype: 'searchfield',
					selectOnFocus: true,
					...
					listeners: {
						afterrender: function( field, eOpts ){
							...
							if(Ext.isEmpty(self.__CXSearchMiniRenderer)){
								self.__CXSearchMiniRenderer = new AgMiniRenderer({width:108,height:108,rate:1});
							}else{
								self.__CXSearchMiniRenderer.domElement.style.display = '';
								self.__CXSearchMiniRenderer.hideAllObj();
							}
							field.layout.innerCt.dom.appendChild( self.__CXSearchMiniRenderer.domElement );
							...
							var loadObj = function(){
								...
								self.__CXSearchMiniRenderer.loadObj(params, function(){
									selModel.on('selectionchange', function(selModel,selected,eOpts){
										...
										self.__CXSearchMiniRenderer.showObj(urls);
									});
...
//							console.log(width, height);
							self.__CXSearchMiniRenderer.setSize(width-6,height-2);
						}
					}
				},{
					flex: 1,
