...
//2016-03-31用 START
//window.AgApp.prototype.regexpIsSubclassCdiName = new RegExp("^([A-Z]+[0-9]+?)\-*([LRP])$", "i");
//window.AgApp.prototype.regexpIsSubclassCdiName = new RegExp("^([A-Z]+[0-9]+?)\-*(L|R|P|S[1-9][0-9]{0,1})$","i");
window.AgApp.prototype.regexpIsSubclassCdiName = new RegExp("^([A-Z]+[0-9]+?)\-*([LRPFN])$","i");
//2016-03-31用 END
...
window.AgApp.prototype.openObjEditWindow = function(config){
	...
	config.height = config.height || 713;
...
//		var gridpanel = (Ext.isArray(fields) ? fields[0] : fields).up('window').down('gridpanel');
		var gridpanel = Ext.getCmp(window_id+'-gridpanel');
		if(!gridpanel) return;

		var name2value = [];
		var set_name2value = function(field){
			var field_name;
			var field_value;
			if(Ext.isObject(field)){
				...
				if(field.getSubmitValue){
					field_value = field.getSubmitValue();
				}else if(Ext.isDefined(field.value)){
					field_value = field.value;
				}
			}

			if(Ext.isEmpty(field_name)) return true;
//			var field_value = field.getSubmitValue();
//			console.log('updateField',name,value);
			if(Ext.isString(field_value)) field_value = Ext.String.trim(field_value);
			if(Ext.isEmpty(field_value)) field_value = null;
			name2value[field_name] = field_value;
		};
		if(Ext.isArray(fields)){
			Ext.each(fields, set_name2value);
		}else if(Ext.isObject(fields)){
			set_name2value(fields);
		}
	...
	var loadSelectedRecords = function(){
		...
		if(Ext.isFunction(cdi_name_field.renderer)){
			setFieldValue(cdi_name_edit_field, cdi_name_field.renderer(cdi_name_field.getValue(),cdi_name_field));
		}else{
			setFieldValue(cdi_name_edit_field, cdi_name_field.getValue());
		}
		...
		if(Ext.isFunction(mirror_cdi_name_field.renderer)){
			...
			setFieldValue(mirror_cdi_name_edit_field, mirror_cdi_name_field.getValue());
		}

		var cmp_id_field = basicForm.findField('cmp_id');
		cmp_id_field.fireEvent('change',cmp_id_field,cmp_id_field.getValue());
	};
	...
	var cdiName = {
		validator: function( value ){
			...
			var displayfield = field.up('fieldset').getComponent('cdi_name_e').getComponent('cdi_name_e');
		...
		change: function(field,newValue,oldValue,prefix,fmaAllStore){
			...
			var mirror_type_mirror_field = formPanel.down('#mirror_type_mirror');
			var mirror_type_same_field = formPanel.down('#mirror_type_same');
			var mirror_id_button = formPanel.down('#mirror_id');

			if(Ext.isEmpty(newValue)){
				cdiName.delete(field,prefix);

				if(Ext.isEmpty(prefix)){
					mirror_type_mirror_field.setBoxLabel(Ext.util.Format.format(mirror_type_mirror_field.format,'-','-','-'));
					mirror_type_mirror_field.setDisabled(true);
					if(!mirror_type_same_field.hidden){
						mirror_type_same_field.setBoxLabel(Ext.util.Format.format(mirror_type_same_field.format,'-','-','-'));
						mirror_type_same_field.setDisabled(true);
					}
				}
			}else{

				if(Ext.isString(newValue)){
					var m = self.regexpIsSubclassCdiName.exec(newValue);
				...
				fmaAllStore = fmaAllStore || Ext.data.StoreManager.lookup('fmaAllStore');
					...
					Ext.Ajax.request({
						...
						callback: function(options,success,response){
							...
							if(!success || Ext.isEmpty(json) || Ext.isEmpty(json.success) || !json.success || Ext.isEmpty(json.datas)){
								...
								if(Ext.isEmpty(prefix)){
									if(!mirror_type_same_field.hidden){
										mirror_type_same_field.setBoxLabel(Ext.util.Format.format(mirror_type_same_field.format, cdi_name_field.renderer(cdi_name_field.getValue(),cdi_name_field),cdi_name_e_field.renderer(cdi_name_e_field.getValue(),cdi_name_e_field)));
										mirror_type_same_field.setDisabled(false);
									}
									if(mirror_id_button) mirror_id_button.fireEvent('click',mirror_id_button);
								}
	...
	var fmaSearch = {
		query: function(b){
			...
			//2016-03-31用 START
			if(Ext.isString(value)){
				var m = self.regexpIsSubclassCdiName.exec(value);
...
//		loadRecords.push(Ext.create(objEditStore.model.modelName,r.data));
		var data = r.getData();
		delete data.target_record;
		loadRecords.push(data);
	});
...
//								if(reader && reader.rawData && reader.rawData.success && Ext.isArray(reader.rawData.datas) && reader.rawData.datas.length){
								if(reader && reader.rawData && reader.rawData.success){
...
*/
									...
									if(self.syncMappingMngStore) self.syncMappingMngStore(store.getRange());
		...
		items: [{
			...
			columns: [
				...
				{text: AgLang.file_name, dataIndex: 'art_filename', flex:1,   minWidth:80, hidden:false, hideable:false},
				{text: AgLang.cdi_name,  dataIndex: 'cdi_name',     width:80, minWidth:80, hidden:false, hideable:false, xtype: 'agcolumncdiname'},
				{text: AgLang.cmp_abbr,  dataIndex: 'cmp_id',       width:40, minWidth:40, hidden:false, hideable:false, xtype: 'agcolumnconceptartmappart'},
				...
				{text: 'type',   dataIndex: 'mirror_type',         width:44, minWidth:44, hidden:true, hideable:false},
...
//					if(Ext.isEmpty(record)) return;
					...
					if(selected.length){
						...
						if(mirrorfield){
//							console.log(mirrorfield.getValue());
//							if(mirrorfield.getValue()) mirrorfield.setValue(false);
...
//							console.log(fieldcontainer.items);
							while(fieldcontainer.items.getCount()){
								fieldcontainer.remove(fieldcontainer.items.getAt(fieldcontainer.items.getCount()-1));
							}
							var buttons = {};
							var button_items = {};
...
//								var text = Ext.String.format('<b>{0}</b>{1}',cmp_title.substr(0,1).toUpperCase(),cmp_title.substr(1).toLowerCase());
								var text = cmp_abbr;
								var tooltip = cmp_title;
...
//									var button_text = Ext.String.format('<b>{0}</b> : {1}',text,tooltip);
									...
									if(!cmp_key_number){
										listeners = {
											afterrender: function( b, eOpts ){
												b.up('fieldcontainer').prev('combobox').on({
													change: function(field,newValue,oldValue,eOpts){
														var store = field.getStore();
														var record = field.findRecordByValue(newValue);
														if(record && record.get('cmp_abbr')===b.itemId){
															if(!b.pressed) b.toggle(true,true);
														}else{
															if(b.pressed) b.toggle(false,true);
														}
													}
												});
											},
											click: function( b, e, eOpts ){
												...
												if(b.pressed){
													...
													var cdi_name_field = form ? form.findField('cdi_name') : null;
													var cdi_name_edit_field = cdi_name_field ? cdi_name_field.nextSibling('textfield') : null;
													var cdi_name = cdi_name_field ? cdi_name_field.getValue() : null;

													var cmp_id = 0;
													var cmp_abbr = '';
													var cmp_record = conceptArtMapPartStore.findRecord('cmp_abbr', b.itemId, 0, false, false, true );
													if(cmp_record){
														cmp_id = cmp_record.get('cmp_id');
														cmp_abbr = cmp_record.get('cmp_abbr');
													}
													b.up('fieldcontainer').prev('combobox').setValue(cmp_id);
													if(cdi_name && cmp_id && (cmp_abbr==='L' || cmp_abbr==='R')){
																...
																if(!success || Ext.isEmpty(json) || Ext.isEmpty(json.success) || !json.success || Ext.isEmpty(json.datas)){
																	...
																	if(data.cdi_name != params.cdi_name){
																		Ext.Msg.show({
																			...
																			fn: function(){
																				cdi_name_edit_field.setValue(data.cdi_name);
																				b.up('fieldcontainer').prev('combobox').setValue(0);
																			}
																		});
																	}
																}
												...
												var mirror_field = b.up('form').getForm().findField('mirror');
												if(mirror_field.getValue()){
													var mirror_id_button = b.up('form').down('button#mirror_id');
													mirror_id_button.fireEvent('click',mirror_id_button);
												}
									...
									buttons[cmp_key] = fieldcontainer.add({
										xtype: 'button',
										text: button_text,
							...
							Ext.Object.each(button_items, function(cmp_key,records){
								...
								if(Ext.isArray(records) && records.length){
									var idx = fieldcontainer.items.findIndex( 'itemId', cmp_key, 0, false, false );
									console.log(idx);
									if(idx>=0){
										var item_datas = [];
										...
										var component_obj = {
											...
											listeners : {
												afterrender: function(combobox){
													var button = combobox.prev('button#'+cmp_key);
													button.on('toggle', function(button, pressed, eOpts){
														combobox.setDisabled(!pressed);
														if(pressed) combobox.fireEvent('change', combobox, combobox.getValue());
													});
													button.on('click', function(button, eOpts){
														if(button.pressed) return;
														button.up('fieldcontainer').prev('combobox').setValue(0);
													});

													combobox.up('fieldcontainer').prev('combobox').on({
														change: function(field,newValue,oldValue,eOpts){

															var record = combobox.findRecordByValue(newValue);

															if(record){
																if(!button.pressed) button.toggle(true,false);
																if(combobox.getValue() != newValue) combobox.setValue(newValue);
															}else{
																if(button.pressed) button.toggle(false,false);
															}
														}
													});
												...
												change: function( combobox, newValue, oldValue, eOpts ){
													var cmp_record = combobox.findRecordByValue(newValue);
													combobox.up('fieldcontainer').prev('combobox').setValue(cmp_record.get('cmp_id'));

													var mirror_field = combobox.up('form').getForm().findField('mirror');
													if(mirror_field.getValue()){
														var mirror_id_button = combobox.up('form').down('button#mirror_id');
														mirror_id_button.fireEvent('click',mirror_id_button);
													}
												},
												enable: function( combobox, eOpts ){
													var button = combobox.prev('button#'+cmp_key);
													if(button){
										...
										if(idx+1<fieldcontainer.items.getCount()){
											fieldcontainer.insert( idx+1, component_obj);
										}else{
											fieldcontainer.add(component_obj);
										}
									}
								}
							});
					...
					defaults: {
						...
						listeners: {
							...
							click: function( b, e, eOpts ){
								...
								if(b.pressed){
									var cmp_record = conceptArtMapPartStore.findRecord('cmp_abbr', b.itemId, 0, false, false, true );
									b.up('fieldcontainer').prev('combobox').setValue(cmp_record.get('cmp_id'));
								}else{
									b.up('fieldcontainer').prev('combobox').setValue(0);
								}
...
*/
					...
					columns: [
						{xtype: 'rownumberer'},
						{text: AgLang.hist_event, dataIndex: 'he_name',        width: 50, minWidth: 50, hidden:false, hideable:false, sortable: false},
						{text: AgLang.cdi_name,   dataIndex: 'cdi_name',       width: 80, minWidth: 80, hidden:false, hideable:false, xtype:'agcolumncdiname'},
						{text: AgLang.cdi_name_e, dataIndex: 'cdi_name_e',     flex: 1,   minWidth: 80, hidden:false, hideable:false, xtype:'agcolumncdinamee'},
						...
						{text: AgLang.user,       dataIndex: 'cm_openid',      width:116, minWidth:116, hidden:true,  hideable:false},
//						{text: AgLang.timestamp,  dataIndex: 'hist_timestamp', width:116, minWidth:116, hidden:false, hideable:false, xtype:'datecolumn', format:self.FORMAT_DATE_TIME}
						{text: AgLang.cm_entry,   dataIndex: 'hist_timestamp', width:136, minWidth:136, hidden: false, hideable: false, xtype: 'datecolumn', format: self.FORMAT_DATE_TIME_M}
					],
					selType: 'rowmodel',
					selModel: {
						mode:'SINGLE'
					...
					listeners: {
						afterrender: function(gridpanel){
							if(Ext.isChrome) gridpanel.columns.forEach(function(c){if(!c.hidden && c.resizable && Ext.isEmpty(c.flex)) c.autoSize()});
//							gridpanel.getStore().on('load',function(){
//								gridpanel.columns.forEach(function(c){if(!c.hidden && c.resizable && Ext.isEmpty(c.flex) && c.autoSize) c.autoSize()});
//							});
						}
					}
				}],
					...
					afterrender: function(field, eOpts){
						...
						el.on({
							...
							drop: function(event){
								...
								field.down('combobox#cdi_name_edit').setValue(dropRecord.get('cdi_name'));
								field.down('agcdinameedisplayfield#cdi_name_e').setValue(dropRecord.get('cdi_name_e'));
							}
						});
					}
				}
				...
				listeners: {
					...
					afterrender: {
						fn: function(field, eOpts){
							...
							field.checkboxCmp.inputValue = true;
					...
					listeners: {
						...
						expand: {
							fn: function(field, eOpts){
								...
								var mirror_id_button = field.down('#mirror_id');
								if(mirror_id_button){
									mirror_id_button.fireEvent('click',mirror_id_button);
...
/**/
									...
									var cdi_name_field = form.findField('cdi_name');
									var cdi_name_e_field = form.findField('cdi_name_e');
									var mirror_type_same_field = formPanel.down('#mirror_type_same');
									if(!mirror_type_same_field.hidden){
										if(cdi_name_field && cdi_name_field.rendered && cdi_name_e_field && cdi_name_e_field.rendered){
											if(Ext.isEmpty(cdi_name_field.getValue()) || Ext.isEmpty(cdi_name_e_field.getValue())){
												mirror_type_same_field.setBoxLabel(Ext.util.Format.format(mirror_type_same_field.format,'-','-','-'));
												mirror_type_same_field.setDisabled(true);
											}else{
												mirror_type_same_field.setBoxLabel(Ext.util.Format.format(mirror_type_same_field.format, cdi_name_field.getValue(), cdi_name_e_field.getValue()));
												mirror_type_same_field.setDisabled(false);
											}
										}
									}
...
*/
						...
						items: [{
							boxLabel: 'represents "contra lateral" FMA (FMA12324 : right ...)',
							format: 'represents "contra lateral" FMA ( {0} : {1} : {2} )',
							name: 'mirror_type',
							itemId: 'mirror_type_mirror',
							inputValue: 'mirror',
							checked: true,
							listeners: {
								afterrender: function(field, eOpts){
									try{field.setBoxLabel(Ext.util.Format.format(field.format,'-','-','-'));}catch(e){}
									var form = field.up('form').getForm();
									var cdi_name_field = form.findField('cdi_name');
									var cdi_name_e_field = form.findField('cdi_name_e');
									if(form && cdi_name_field && cdi_name_e_field){
										form.on({
											dirtychange: function(){
												if(Ext.isEmpty(cdi_name_field.getValue()) || Ext.isEmpty(cdi_name_e_field.getValue())){
													field.setBoxLabel(Ext.util.Format.format(field.format,'-','-','-'));
...
//													field.setDisabled(false);
								...
								change: function(field, newValue, oldValue, eOpts){
									if(newValue){
										var f = field.up('#mirror_type').next('#cdi_name');
										if(f){
										...
										updateField(field);
										var mirror_id_button = field.up('form').down('button#mirror_id');
										if(mirror_id_button) mirror_id_button.fireEvent('click',mirror_id_button);
									}
								},
								disable: function(field,eOpts){
									if(field.getValue()) updateField({name:'mirror_type',value:null});
								},
								enable: function(field,eOpts){
									if(field.getValue()) updateField({name:'mirror_type',value:'mirror'});
								}
							}
						},{
							hidden: true,
							boxLabel: 'is part of same FMA ( FMA12323 : left...)',
							format: 'is part of same FMA ( {0} : {1}  : {2} )',
							name: 'mirror_type',
							itemId: 'mirror_type_same',
							inputValue: 'same',
							listeners: {
								afterrender: function(field, eOpts){
									if(field.hidden) return;
										...
										form.on({
											dirtychange: function(){
												if(Ext.isEmpty(cdi_name_field.getValue()) || Ext.isEmpty(cdi_name_e_field.getValue())){
													...
													field.setBoxLabel(Ext.util.Format.format(field.format, cdi_name_field.renderer(cdi_name_field.getValue(),cdi_name_field),cdi_name_e_field.renderer(cdi_name_e_field.getValue(),cdi_name_e_field)));
													field.setDisabled(false);
												}
											},
											validitychange: function(){
								...
								change: function(field, newValue, oldValue, eOpts){
									var form = field.up('form').getForm();
									var same_concept_field = form.findField(mirror_prefix+'same_concept');
									same_concept_field.setValue(newValue);
									if(newValue){
										var f = field.up('#mirror_type').next('#cdi_name');
										if(f){
											f.setDisabled(newValue);
								...
								enable: function(field,eOpts){
									if(field.getValue()) updateField({name:'mirror_type',value:'same'});
								}
							}
						},{
							boxLabel: 'other FMA',
							name: 'mirror_type',
							itemId: 'mirror_type_other',
							inputValue: 'other',
							listeners: {
								change: function(field, newValue, oldValue, eOpts){
									if(newValue){
...
//							name: mirror_prefix+'cdi_name',			//2016-03-31用 COMMENT OUT
							...
							listeners: {
								focus: cdiName.focus,
								change: function(field,newValue,oldValue,eOpts){
									cdiName.change(field,newValue,oldValue,mirror_prefix,Ext.data.StoreManager.lookup('fmaMirrorAllStore'));
								},
								beforequery: cdiName.beforeQuery,
								select: function( field, records, eOpts ){
									return;
							...
							listeners: {
								click: function(b,e,eOpts){
									...
									var mirror_type_mirror = b.up('#concept').down('#mirror_type_mirror');
									if(mirror_type_mirror){
										if(mirror_type_mirror.format) mirror_type_mirror.setBoxLabel(Ext.util.Format.format(mirror_type_mirror.format,'-','-','-'));
										mirror_type_mirror.setDisabled(true);
									}
									...
									if(cmp_record && (cmp_record.get('cmp_abbr')==='L' || cmp_record.get('cmp_abbr')==='R')){
										...
										if(Ext.isFunction(mirror_cdi_name_field.renderer)){
											...
											setFieldValue(mirror_cdi_name_edit_field, cdi_name_field.getValue());
										}
										setFieldValue(mirror_cdi_name_field, cdi_name_field.getValue());
										fields.push(mirror_cdi_name_field);
										...
										if(mirror_type_mirror){
											if(mirror_type_mirror.format){
												if(Ext.isFunction(mirror_cdi_name_e_field.renderer)){
													mirror_type_mirror.setBoxLabel(Ext.util.Format.format(mirror_type_mirror.format,mirror_cdi_name_edit_field.getValue(),mirror_cmp_record.get('cmp_abbr'),mirror_cdi_name_e_field.renderer(mirror_cdi_name_e_field.getValue(),mirror_cdi_name_e_field)));
												}else{
													mirror_type_mirror.setBoxLabel(Ext.util.Format.format(mirror_type_mirror.format,mirror_cdi_name_edit_field.getValue(),mirror_cmp_record.get('cmp_abbr'),mirror_cdi_name_e_field.getValue()));
												}
											}
											mirror_type_mirror.setDisabled(false);
										}
									}
									else{
...
//											Ext.Msg.alert(b.text, 'None Data');
											...
											if(mirror_type_mirror){
												if(mirror_type_mirror.format) mirror_type_mirror.setBoxLabel('<label style="color:red;">'+Ext.util.Format.format(mirror_type_mirror.format,AgLang.cdi_name,AgLang.not_found)+'</label>');
											}
											var basicForm = b.up('form').getForm();
											Ext.each(['cdi_name','cdi_name_e'],function(name,index,arr){
...
//													Ext.Msg.alert(b.text, 'None Data');
												...
												if(mirror_type_mirror){
													if(mirror_type_mirror.format){
														if(cmp_record){
															mirror_type_mirror.setBoxLabel(Ext.util.Format.format(mirror_type_mirror.format,record.get('cdi_name'),cmp_record.get('cmp_abbr'),record.get('cdi_name_e')));
														}else{
															mirror_type_mirror.setBoxLabel(Ext.util.Format.format(mirror_type_mirror.format,record.get('cdi_name'),'-',record.get('cdi_name_e')));
														}
													}
													mirror_type_mirror.setDisabled(false);
												}
					...
					fmaAllStore.on({
						load: function(){
							...
							if(selModel.getSelectionMode() == 'MULTI'){
								...
								selModel.select(gridpanel.getStore().getAt(0));
							}
