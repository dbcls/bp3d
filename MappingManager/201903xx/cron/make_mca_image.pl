...
#say __LINE__.':'.join("\n",sort {$a cmp $b} map {$CDI_ID2NAME{$_}} keys(%$current_set));
#exit;
#=pod
...
#		}
		...
		foreach my $cdi_id (keys(%$CDI_ID2CID)){
			...
			foreach my $cdi_cid (@{$CDI_ID2CID->{$cdi_id}}){
				...
				foreach my $art_id (keys(%{$CDI_MAP_ART_DATE->{$cdi_cid}})){
					$CDI_MAP_ART_DATE->{$cdi_id}->{$art_id} = undef;
				}
			}
		}
=pod
unless(exists $CDI_ID2CID->{$CDI_NAME2ID{'FMA20394'}}){
	say __LINE__;
}else{
	say __LINE__;
}
unless(exists $CDI_MAP_ART_DATE->{$CDI_NAME2ID{'FMA20394'}}){
	say __LINE__;
}else{
	say __LINE__;
}
...
#exit;
			...
			foreach my $t_b_id (sort {$a->{'cdi_name'} cmp $b->{'cdi_name'}} @CDI_NAMES){
				...
				my $crl_id;
				my $art_ids_md5;
#				my $sth_mca = $dbh->prepare(qq|select cmm.mca_id,EXTRACT(EPOCH FROM mca_entry),crl_id,md5(art_ids_arr) from concept_art_map_modified as cmm,mapping_concepts_arts as mca where cm_delcause is null and cmm.mca_id is not null and cmm.mca_id=mca.mca_id and md_id=$md_id and mv_id=$mv_id and cdi_id=?|) or die $dbh->errstr;
				my $sth_mca = $dbh->prepare(qq|select cmm.mca_id,EXTRACT(EPOCH FROM mca_entry),crl_id,md5(art_ids_arr) from concept_art_map_modified as cmm,mapping_concepts_arts as mca where cm_delcause is null and cmm.mca_id is not null and cmm.mca_id=mca.mca_id and md_id=$md_id and mv_id=$mv_id and cdi_id=? and crl_id=0|) or die $dbh->errstr;
				$sth_mca->execute($CDI_NAME2ID{$target_cdi_name}) or die $dbh->errstr;
				$column_number = 0;
				$sth_mca->bind_col(++$column_number, \$mca_id, undef);
				$sth_mca->bind_col(++$column_number, \$mca_entry, undef);
				$sth_mca->bind_col(++$column_number, \$crl_id, undef);
				$sth_mca->bind_col(++$column_number, \$art_ids_md5, undef);
				while($sth_mca->fetch){
					$MCA{$mca_id} = {
						mca_entry => $mca_entry - 0,
						crl_id => $crl_id - 0,
						art_ids_md5 => $art_ids_md5
					};
				}
				$sth_mca->finish;
				undef $sth_mca;

				foreach my $mca_id (sort keys(%MCA)){
					$rep_entry = $MCA{$mca_id}->{'mca_entry'};
					$crl_id = $MCA{$mca_id}->{'crl_id'};
					$art_ids_md5 = $MCA{$mca_id}->{'art_ids_md5'};

#say $target_cdi_name."\t".$crl_id."\t".$art_ids_md5;

#					my($img_prefix,$img_path) = &getMcaImagePrefix($mca_id);
					my($img_prefix,$img_path) = &getMcaImagePrefix($art_ids_md5);
...
#exit;
									...
									foreach my $art_file (@$art_files){
										...
										unless(exists $CDI_MAP_ART_DATE->{$cdi_id} && defined $CDI_MAP_ART_DATE->{$cdi_id} && exists $CDI_MAP_ART_DATE->{$cdi_id}->{$art_id}){
#&cgi_lib::common::message($CDI_MAP_ART_DATE->{$cdi_id});
											next;
										}
...
#&cgi_lib::common::message($obj_files1);
...
#								$tmp_img_prefix = &catdir($tmp_base_path,$mca_id);
								$tmp_img_prefix = &catdir($tmp_base_path,$art_ids_md5);
...
#								symlink qq|$mca_id\_$sizeStrM.gif|,qq|$img_prefix.gif| if(-e $imgsM->[0]);
								symlink qq|$art_ids_md5\_$sizeStrM.gif|,qq|$img_prefix.gif| if(-e $imgsM->[0]);
