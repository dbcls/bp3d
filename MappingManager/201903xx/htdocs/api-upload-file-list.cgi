...
my $time2pid_file = &catdir($FindBin::Bin,'temp','time2pid.json');
my $time2pid;
if(-e $time2pid_file && -f $time2pid_file && -r $time2pid_file && -s $time2pid_file){
	$time2pid = &readFileJSON($time2pid_file);
}else{
	$time2pid = {};
}
...
if(
	...
	if(
		exists	$FORM{'_timeid'} &&
		defined	$FORM{'_timeid'} &&
		length	$FORM{'_timeid'} &&
		exists	$time2pid->{$FORM{'_timeid'}} &&
		defined	$time2pid->{$FORM{'_timeid'}} &&
		length	$time2pid->{$FORM{'_timeid'}}
	){
		$DATAS->{'success'} = JSON::XS::true;
		if(kill(0,$time2pid->{$FORM{'_timeid'}})){
			$DATAS->{'total'}++;
			kill('KILL',$time2pid->{$FORM{'_timeid'}}) ;
		}
		delete $time2pid->{$FORM{'_timeid'}};
		&writeFileJSON($time2pid_file,$time2pid);
	}
...
#			my $sth_cm_sel = $dbh->prepare(qq|select cdi_id,cm_comment,cmp_id from concept_art_map where art_id=? and ci_id=?|) or die $dbh->errstr;
#			my $sth_cm_upd = $dbh->prepare(qq|update concept_art_map set cdi_id=?,cm_comment=?,cmp_id=?,cm_entry=now(),cm_delcause=null where art_id=? and ci_id=?|) or die $dbh->errstr;
#			my $sth_cm_ins = $dbh->prepare(qq|insert into concept_art_map (cdi_id,cm_comment,cmp_id,art_id,ci_id) values (?,?,?,?,?)|) or die $dbh->errstr;
#			my $sth_cm_del = $dbh->prepare(qq|delete from concept_art_map where art_id=? and ci_id=?|) or die $dbh->errstr;
			my $sth_cm_sel = $dbh->prepare(qq|select cdi_id,cm_comment,cmp_id,cm_use from concept_art_map where art_id=? and md_id=$md_id and mv_id=$mv_id|) or die $dbh->errstr;
#			my $sth_cm_upd = $dbh->prepare(qq|update concept_art_map set cdi_id=?,cm_comment=?,cmp_id=?,cm_entry=now(),cm_delcause=null where art_id=? and md_id=$md_id and mv_id=$mv_id|) or die $dbh->errstr;
			my $sth_cm_upd = $dbh->prepare(qq|update concept_art_map set cdi_id=?,cm_comment=?,cmp_id=?,cm_use=true,cm_entry=now(),cm_delcause=null where art_id=? and md_id=$md_id and mv_id=$mv_id|) or die $dbh->errstr;
#			my $sth_cm_ins = $dbh->prepare(qq|insert into concept_art_map (cdi_id,cm_comment,cmp_id,art_id,ci_id,cb_id,md_id,mv_id) values (?,?,?,?,$ci_id,$cb_id,$md_id,$mv_id)|) or die $dbh->errstr;
			my $sth_cm_ins = $dbh->prepare(qq|insert into concept_art_map (cdi_id,cm_comment,cmp_id,art_id,ci_id,md_id,mv_id) values (?,?,?,?,$ci_id,$md_id,$mv_id)|) or die $dbh->errstr;
			my $sth_cm_del = $dbh->prepare(qq|delete from concept_art_map where art_id=? and md_id=$md_id and mv_id=$mv_id|) or die $dbh->errstr;
...
#						print $LOG __LINE__.qq|:\$rows=[$rows]\n|;
				...
				$sth_cm_sel->execute($art_id) or die $dbh->errstr;
				$rows = $sth_cm_sel->rows();
				print $LOG __LINE__.qq|:\$rows=[$rows]\n|;
				my($map_cdi_id,$cm_comment,$cmp_id,$cm_use);
				if($rows>0){
					my $column_number = 0;
					$sth_cm_sel->bind_col(++$column_number, \$map_cdi_id, undef);
					$sth_cm_sel->bind_col(++$column_number, \$cm_comment, undef);
					$sth_cm_sel->bind_col(++$column_number, \$cmp_id, undef);
					$sth_cm_sel->bind_col(++$column_number, \$cm_use, undef);
					$sth_cm_sel->fetch;
				}
				$sth_cm_sel->finish;
				...
				unless(
					...
					if($rows>0){
						if(defined $cdi_id){
							$sth_cm_upd->execute($cdi_id,$data->{'cm_comment'},$data->{'cmp_id'},$art_id,) or die $dbh->errstr;
							$sth_cm_upd->finish;
						}else{
							$sth_cm_del->execute($art_id) or die $dbh->errstr;
							$sth_cm_del->finish;
						}
					}elsif(defined $cdi_id){
						$sth_cm_ins->execute($cdi_id,$data->{'cm_comment'},$data->{'cmp_id'},$art_id) or die $dbh->errstr;
						$sth_cm_ins->finish;
					}
...
#								elsif(defined $data->{'mirror_cdi_name'} && length $data->{'mirror_cdi_name'}){
								...
								$sth_cm_sel->execute($mirror_art_id) or die $dbh->errstr;
								my $rows = $sth_cm_sel->rows();
								my($map_cdi_id,$cm_comment,$cmp_id,$cm_use);
								if($rows>0){
									my $column_number = 0;
									$sth_cm_sel->bind_col(++$column_number, \$map_cdi_id, undef);
								...
								unless(
									...
									if($rows>0){
										if(defined $mirror_cdi_id){
											$sth_cm_upd->execute($mirror_cdi_id,$mirror_cm_comment,$mirror_cmp_id,$mirror_art_id) or die $dbh->errstr;
											$sth_cm_upd->finish;
										}else{
											$sth_cm_del->execute($mirror_art_id,$ci_id) or die $dbh->errstr;
											$sth_cm_del->finish;
										}
									}elsif(defined $mirror_cdi_id){
										$sth_cm_ins->execute($mirror_cdi_id,$mirror_cm_comment,$mirror_cmp_id,$mirror_art_id) or die $dbh->errstr;
										$sth_cm_ins->finish;
									}
...
=cut
	...
	if(defined $datas && ref $datas eq 'ARRAY'){
		...
		$dbh->{'RaiseError'} = 1;
		eval{
#			my $sth_cm_sel = $dbh->prepare(qq|select cdi_id from concept_art_map where art_id=? and ci_id=$ci_id|) or die $dbh->errstr;
#			my $sth_cm_ins = $dbh->prepare(qq|insert into concept_art_map (cdi_id,cmp_id,art_id,ci_id) values (?,?,?,$ci_id)|) or die $dbh->errstr;
#			my $sth_cm_upd = $dbh->prepare(qq|update concept_art_map set cdi_id=?,cmp_id=?,cm_delcause=null,cm_entry=now() where art_id=? and ci_id=$ci_id|) or die $dbh->errstr;
#			my $sth_cm_del = $dbh->prepare(qq|delete from concept_art_map where art_id=? and ci_id=$ci_id|) or die $dbh->errstr;

			my $sth_cm_sel = $dbh->prepare(qq|select cdi_id from concept_art_map where art_id=? and md_id=$md_id and mv_id=$mv_id|) or die $dbh->errstr;
#			my $sth_cm_ins = $dbh->prepare(qq|insert into concept_art_map (cdi_id,cmp_id,art_id,ci_id,cb_id,md_id,mv_id) values (?,?,?,$ci_id,$cb_id,$md_id,$mv_id)|) or die $dbh->errstr;
			my $sth_cm_ins = $dbh->prepare(qq|insert into concept_art_map (cdi_id,cmp_id,art_id,ci_id,md_id,mv_id) values (?,?,?,$ci_id,$md_id,$mv_id)|) or die $dbh->errstr;
#			my $sth_cm_upd = $dbh->prepare(qq|update concept_art_map set cdi_id=?,cmp_id=?,cm_delcause=null,cm_entry=now() where art_id=? and md_id=$md_id and mv_id=$mv_id|) or die $dbh->errstr;
			my $sth_cm_upd = $dbh->prepare(qq|update concept_art_map set cdi_id=?,cmp_id=?,cm_use=?,cm_delcause=null,cm_entry=now() where art_id=? and md_id=$md_id and mv_id=$mv_id|) or die $dbh->errstr;
			my $sth_cm_del = $dbh->prepare(qq|delete from concept_art_map where art_id=? and md_id=$md_id and mv_id=$mv_id|) or die $dbh->errstr;
			...
			my $sth_cm_cdi_sel = $dbh->prepare(qq|select art_id from concept_art_map where cdi_id=? and md_id=$md_id and mv_id=$mv_id|) or die $dbh->errstr;
			my $sth_cm_exists_sel = $dbh->prepare(qq|select art_id from concept_art_map where cdi_id=? and cmp_id=? and cm_use=? and art_id=? and md_id=$md_id and mv_id=$mv_id|) or die $dbh->errstr;
...
SQL
			...
			my $sth_cp_abbr_sel = $dbh->prepare(qq|select cp_abbr from concept_part where cp_id=?|) or die $dbh->errstr;
			my $sth_cp_id_sel = $dbh->prepare(qq|select cp_id from concept_part where cp_abbr=?|) or die $dbh->errstr;

			my $sth_cl_abbr_sel = $dbh->prepare(qq|select cl_abbr from concept_laterality where cl_id=?|) or die $dbh->errstr;
			my $sth_cl_id_sel = $dbh->prepare(qq|select cl_id from concept_laterality where cl_abbr=?|) or die $dbh->errstr;

			my $sth_artl_abbr_sel = $dbh->prepare(qq|select artl_abbr from art_laterality where artl_id=?|) or die $dbh->errstr;
			my $sth_artl_id_sel = $dbh->prepare(qq|select artl_id from art_laterality where artl_abbr=?|) or die $dbh->errstr;
			my $sth_arti_artl_upd = $dbh->prepare('UPDATE art_file_info SET artl_id=?,artc_id=?,art_modified=now() WHERE art_id=?') or die $dbh->errstr;
			my $sth_arti_artl_upd_def = $dbh->prepare('UPDATE art_file_info SET artl_id=DEFAULT,artc_id=DEFAULT,art_modified=now() WHERE art_id=?') or die $dbh->errstr;
...
#					}
#				}
				if(($data->{'cp_id'} || $data->{'cl_id'}) && $data->{'cdi_name'} !~ /$is_subclass_cdi_name/){
					my $cp_abbr;
					my $cl_abbr;
					if($data->{'cp_id'}){
						$sth_cp_abbr_sel->execute($data->{'cp_id'}) or die $dbh->errstr;
					...
					if($data->{'cl_id'}){
						$sth_cl_abbr_sel->execute($data->{'cl_id'}) or die $dbh->errstr;
					...
					if(defined $cp_abbr && length $cp_abbr){
						$data->{'cdi_name'} = sprintf('%s%s',$data->{'cdi_name'},$cp_abbr);
						$data->{'cp_id'} = 0;
					}
					if(defined $cl_abbr && length $cl_abbr){
						$data->{'cdi_name'} = sprintf('%s-%s',$data->{'cdi_name'},$cl_abbr);
						$data->{'cl_id'} = 0;
					}
				}
				...
				my $cp_id = (exists $data->{'cp_id'} && defined $data->{'cp_id'}) ? $data->{'cp_id'}-0 : 0;
				my $cl_id = (exists $data->{'cl_id'} && defined $data->{'cl_id'}) ? $data->{'cl_id'}-0 : 0;
...
#				if(defined $cdi_name && length $cdi_name && exists $data->{'cm_use'} && defined $data->{'cm_use'} && $data->{'cm_use'}){
				...
				if(defined $cdi_id){
					if($sel_rows>0){
#						$sth_cm_exists_sel->execute($cdi_id,$cmp_id,$art_id) or die $dbh->errstr;
						$sth_cm_exists_sel->execute($cdi_id,$cmp_id,$cm_use,$art_id) or die $dbh->errstr;
						$sel_rows = $sth_cm_exists_sel->rows();
						$sth_cm_exists_sel->finish;
						unless($sel_rows>0){
#							$sth_cm_upd->execute($cdi_id,$cmp_id,$art_id) or die $dbh->errstr;
							$sth_cm_upd->execute($cdi_id,$cmp_id,$cm_use,$art_id) or die $dbh->errstr;
							$sth_cm_upd->finish;
						}
					}else{
						$sth_cm_ins->execute($cdi_id,$cmp_id,$art_id) or die $dbh->errstr;
						$sth_cm_ins->finish;
					}
...
#					}
					...
					if($cdi_name =~ /$is_subclass_cdi_name/){
						...
						my $cl_abbr = $3;
						if(defined $cp_abbr && length $cp_abbr){
							$sth_cp_id_sel->execute($cp_abbr) or die $dbh->errstr;
							if($sth_cp_id_sel->rows()>0){
								$column_number = 0;
								$sth_cp_id_sel->bind_col(++$column_number, \$cp_id, undef);
								$sth_cp_id_sel->fetch;
							}
							$sth_cp_id_sel->finish;
						}
						else{
							$cp_id = 0;
						}
						if(defined $cl_abbr && length $cl_abbr){
							$sth_cl_id_sel->execute($cl_abbr) or die $dbh->errstr;
							if($sth_cl_id_sel->rows()>0){
								$column_number = 0;
								$sth_cl_id_sel->bind_col(++$column_number, \$cl_id, undef);
								$sth_cl_id_sel->fetch;
							}
							$sth_cl_id_sel->finish;

							$sth_artl_id_sel->execute($cl_abbr) or die $dbh->errstr;
							if($sth_artl_id_sel->rows()>0){
								$column_number = 0;
								$sth_artl_id_sel->bind_col(++$column_number, \$artl_id, undef);
								$sth_artl_id_sel->fetch;
							}
							$sth_artl_id_sel->finish;
							if(defined $artl_id && length $artl_id){
								my $artl_abbr;
								$sth_artl_abbr_sel->execute($artl_id) or die $dbh->errstr;
								if($sth_artl_abbr_sel->rows()>0){
									$column_number = 0;
									$sth_artl_abbr_sel->bind_col(++$column_number, \$artl_abbr, undef);
									$sth_artl_abbr_sel->fetch;
								}
								$sth_artl_abbr_sel->finish;
								$artc_id = $art_id.($artl_abbr||'');
								if($artc_id ne $art_id){
									$sth_arti_artl_upd->execute($artl_id,$artc_id,$art_id) or die $dbh->errstr;
									$sth_arti_artl_upd->finish;
								}
								else{
									$sth_arti_artl_upd_def->execute($art_id) or die $dbh->errstr;
									$sth_arti_artl_upd_def->finish;
								}
							}
						}
						else{
					...
					if(defined $cdi_name_e && length $cdi_name_e && exists $data->{'mirror'} && defined $data->{'mirror'} && $data->{'mirror'}){
						...
						my $mirror_cp_id  = (exists $data->{'mirror_cp_id'}  && defined $data->{'mirror_cp_id'}  && length $data->{'mirror_cp_id'}  ? $data->{'mirror_cp_id'}  : 0 );
						my $mirror_cl_id  = (exists $data->{'mirror_cl_id'}  && defined $data->{'mirror_cl_id'}  && length $data->{'mirror_cl_id'}  ? $data->{'mirror_cl_id'}  : 0 );
						my $mirror_art_id = (exists $data->{'mirror_art_id'} && defined $data->{'mirror_art_id'} && length $data->{'mirror_art_id'} ? $data->{'mirror_art_id'} : undef);
						my $art_mirroring;
...
#							}
							...
							if(($data->{'mirror_cp_id'} || $data->{'mirror_cl_id'}) && $data->{'mirror_cdi_name'} !~ /$is_subclass_cdi_name/){
								my $cp_abbr;
								my $cl_abbr;
								if($data->{'mirror_cp_id'}){
									$sth_cp_abbr_sel->execute($data->{'mirror_cp_id'}) or die $dbh->errstr;
									if($sth_cp_abbr_sel->rows()>0){
										$column_number = 0;
										$sth_cp_abbr_sel->bind_col(++$column_number, \$cp_abbr, undef);
										$sth_cp_abbr_sel->fetch;
								...
								if($data->{'mirror_cl_id'}){
									$sth_cl_abbr_sel->execute($data->{'mirror_cl_id'}) or die $dbh->errstr;
									if($sth_cl_abbr_sel->rows()>0){
										$column_number = 0;
										$sth_cl_abbr_sel->bind_col(++$column_number, \$cl_abbr, undef);
										$sth_cl_abbr_sel->fetch;
								...
								if(defined $cp_abbr && length $cp_abbr){
									$data->{'mirror_cdi_name'} = sprintf('%s%s',$data->{'mirror_cdi_name'},$cp_abbr);
									$data->{'mirror_cp_id'} = 0;
								}
								if(defined $cl_abbr && length $cl_abbr){
									$data->{'mirror_cdi_name'} = sprintf('%s-%s',$data->{'mirror_cdi_name'},$cl_abbr);
									$data->{'mirror_cl_id'} = 0;
								}
							}
							...
							$mirror_cp_id  = (exists $data->{'mirror_cp_id'}  && defined $data->{'mirror_cp_id'}  && length $data->{'mirror_cp_id'}  ? $data->{'mirror_cp_id'}  : 0 );
							$mirror_cl_id  = (exists $data->{'mirror_cl_id'}  && defined $data->{'mirror_cl_id'}  && length $data->{'mirror_cl_id'}  ? $data->{'mirror_cl_id'}  : 0 );
...
=cut
						...
						if(defined $mirror_cdi_id && defined $mirror_art_id){
							...
							if($sel_rows>0){
#								$sth_cm_exists_sel->execute($mirror_cdi_id,$mirror_cmp_id,$mirror_art_id) or die $dbh->errstr;
								$sth_cm_exists_sel->execute($mirror_cdi_id,$mirror_cmp_id,$cm_use,$mirror_art_id) or die $dbh->errstr;
								$sel_rows = $sth_cm_exists_sel->rows();
								$sth_cm_exists_sel->finish;
								&cgi_lib::common::message('$sel_rows='.$sel_rows, $LOG) if(defined $LOG);
								unless($sel_rows>0){
									$sth_cm_upd->execute($mirror_cdi_id,$mirror_cmp_id,$cm_use,$mirror_art_id) or die $dbh->errstr;
									$sth_cm_upd->finish;
								}
							}else{
								$sth_cm_ins->execute($mirror_cdi_id,$mirror_cmp_id,$mirror_art_id) or die $dbh->errstr;
								$sth_cm_ins->finish;
							}
...
#							}
							...
							my $mirror_artl_id = 0;
							my $mirror_artc_id = undef;

							if($mirror_cdi_name =~ /$is_subclass_cdi_name/){
								$mirror_cdi_name = $1;
								my $mirror_cp_abbr = $2;
								my $mirror_cl_abbr = $3;
								if(defined $mirror_cp_abbr && length $mirror_cp_abbr){
									$sth_cp_id_sel->execute($mirror_cp_abbr) or die $dbh->errstr;
									if($sth_cp_id_sel->rows()>0){
										$column_number = 0;
										$sth_cp_id_sel->bind_col(++$column_number, \$mirror_cp_id, undef);
										$sth_cp_id_sel->fetch;
									}
									$sth_cp_id_sel->finish;
								}
								...
								if(defined $mirror_cl_abbr && length $mirror_cl_abbr){
									$sth_cl_id_sel->execute($mirror_cl_abbr) or die $dbh->errstr;
									if($sth_cl_id_sel->rows()>0){
										$column_number = 0;
										$sth_cl_id_sel->bind_col(++$column_number, \$mirror_cl_id, undef);
										$sth_cl_id_sel->fetch;
									}
									$sth_cl_id_sel->finish;

									$sth_artl_id_sel->execute($mirror_cl_abbr) or die $dbh->errstr;
									if($sth_artl_id_sel->rows()>0){
										$column_number = 0;
										$sth_artl_id_sel->bind_col(++$column_number, \$mirror_artl_id, undef);
										$sth_artl_id_sel->fetch;
									}
									$sth_artl_id_sel->finish;
									if(defined $mirror_artl_id && length $mirror_artl_id){
										my $mirror_artl_abbr;
										$sth_artl_abbr_sel->execute($mirror_artl_id) or die $dbh->errstr;
										if($sth_artl_abbr_sel->rows()>0){
											$column_number = 0;
											$sth_artl_abbr_sel->bind_col(++$column_number, \$mirror_artl_abbr, undef);
											$sth_artl_abbr_sel->fetch;
										}
										$sth_artl_abbr_sel->finish;
										$mirror_artc_id = $mirror_art_id.($mirror_artl_abbr||'');
										if($mirror_artc_id ne $mirror_art_id){
											$sth_arti_artl_upd->execute($mirror_artl_id,$mirror_artc_id,$mirror_art_id) or die $dbh->errstr;
											$sth_arti_artl_upd->finish;
										}
										else{
											$sth_arti_artl_upd_def->execute($mirror_art_id) or die $dbh->errstr;
											$sth_arti_artl_upd_def->finish;
										}
									}
								}
							...
							push(@{$DATAS->{'datas'}},{
								...
								cp_id      => defined $mirror_cp_id ? $mirror_cp_id - 0 : undef,
								cl_id      => defined $mirror_cl_id ? $mirror_cl_id - 0 : undef,
							});
						}
...
#			$dbh->rollback;
			...
			if(scalar @{$DATAS->{'datas'}}){
				...
				foreach my $data (@{$DATAS->{'datas'}}){
					...
					$cdi_name = sprintf("%s%s",$cdi_name,$cp_abbr) if(defined $cp_abbr && length $cp_abbr);
					$cdi_name = sprintf("%s-%s",$cdi_name,$cl_abbr) if(defined $cl_abbr && length $cl_abbr);
					push(@$cdi_names, $cdi_name);
				}
				undef $sth_cp_abbr_sel;
				undef $sth_cl_abbr_sel;

				&recreate_subclass(%FORM, cdi_name => $cdi_names,  LOG => $LOG);
			}
			else{
				&recreate_subclass(%FORM, LOG => $LOG);
			}
...
#				$where_artf_id = 'COALESCE(aff.artf_id,0)='.$artf_id;
			...
			if(defined $cdi_name && length $cdi_name){
				...
				$sth = $dbh->prepare(qq|SELECT cdi.cdi_id,cd_name FROM concept_data_info as cdi LEFT JOIN (SELECT * FROM concept_data WHERE ci_id=$ci_id AND cb_id=$cb_id) cd ON cd.cdi_id=cdi.cdi_id WHERE cdi.ci_id=$ci_id AND cdi_name=?|) or die $dbh->errstr;
...
-- arti.art_cube_volume,
 ...
 arti.artl_id as artl_id,
 artl.artl_abbr as artl_abbr,
 arti.artc_id as artc_id,
...
left join art_laterality AS artl ON arti.artl_id=artl.artl_id
...
#				$sth->bind_col(++$column_number, \$art_cube_volume, undef);
				...
				$sth->bind_col(++$column_number, \$artl_id, undef);
...
#					&cgi_lib::common::message($current_use, $LOG) if(defined $LOG);
					...
					&cgi_lib::common::message(sprintf("%s\t%s",defined $art_id ? $art_id : '',defined $cdi_name ? $cdi_name : ''), $LOG) if(defined $LOG);
...
=cut
						...
						my $cp_abbr = $2;
						my $cl_abbr = $3;
						&cgi_lib::common::message(sprintf("%s\t%s\t%s",$cdi_name,$cp_abbr,$cl_abbr), $LOG) if(defined $LOG);

						$sth_cp_id_sel->execute($cp_abbr) or die $dbh->errstr;
						my $cp_rows = $sth_cp_id_sel->rows();
						my $hash_cp;
						$hash_cp = $sth_cp_id_sel->fetchrow_hashref if($cp_rows>0);
						$sth_cp_id_sel->finish;
						$cp_id = $hash_cp->{'cp_id'} if(defined $hash_cp && ref $hash_cp eq 'HASH' && exists $hash_cp->{'cp_id'} && defined $hash_cp->{'cp_id'});
						undef $hash_cp;


						$sth_cl_id_sel->execute($cl_abbr) or die $dbh->errstr;
						my $cl_rows = $sth_cl_id_sel->rows();
						my $hash_cl;
						$hash_cl = $sth_cl_id_sel->fetchrow_hashref if($cl_rows>0);
						$sth_cl_id_sel->finish;
						$cl_id = $hash_cl->{'cl_id'} if(defined $hash_cl && ref $hash_cl eq 'HASH' && exists $hash_cl->{'cl_id'} && defined $hash_cl->{'cl_id'});
						undef $hash_cl;
					}
					...
					push(@{$DATAS->{'datas'}},{
						...
						artl_id         => defined $artl_id ? $artl_id - 0 : undef,
						artl_abbr       => $artl_abbr,
						artc_id         => $artc_id,
...
#						cmp_id         => (defined $cmp_id ? $cmp_id-0 : 0),
						cp_id          => (defined $cp_id ? $cp_id-0 : 0),
						cl_id          => (defined $cl_id ? $cl_id-0 : 0),
		...
		if(exists $FORM{'cdi_name'} && defined $FORM{'cdi_name'}){
			if(ref $FORM{'cdi_name'} eq 'ARRAY'){
				foreach my $cdi_name (@{$FORM{'cdi_name'}}){
					push(@OPTIONS, sprintf('--cdi_name=%s',$cdi_name));
				}
			}
			elsif(ref $FORM{'cdi_name'} eq ''){
				push(@OPTIONS, sprintf('--cdi_name=%s',$FORM{'cdi_name'}));
			}
		}
		my $options = join(' ',@OPTIONS);
