...
use constant {
	...
	USE_INSTANTIATED_RELATIONS => 0,	#trioの親関係を使用する
	USE_ISA_TREE => 1,	#is_aの親関係を使用する
...
my $sql_relation=<<SQL;
select crt_id,crt_name from concept_relation_type
SQL
...
if(exists $config->{'lexicalsuper'}){# && defined $config->{'lexicalsuper'} && length $config->{'lexicalsuper'}){
	if(defined $config->{'lexicalsuper'} && length $config->{'lexicalsuper'} && -e $config->{'lexicalsuper'} && -f $config->{'lexicalsuper'} && -s $config->{'lexicalsuper'} && -r $config->{'lexicalsuper'}){
		my $relationship = 'lexicalsuper';
		my $p_type = $PARTOF_TYPE->{$relationship};

		my $lexicalsuper_path = $config->{'lexicalsuper'};
...
#				say STDERR qq|exclude:[$cdi_full_name][$cdi_name_e] -> [$cdi_full_pname][$cdi_pname_e]|;
#				next;
#			}
=pod
最新除外パターン(Feb03,2022)
　集合 of :  complex|tree|network|set|
　付随 of :  disk|valve|ligament|artery|vein|duct|mater|joint
　人名 of:   Giacomini|Guyon|Carabelli|Clarke|Ecker|Calleja|Schwalbe|Zinn

(注）tributary of A, branch of A はAの部分なので除外しない　A = branch of A + trunk of A
=cut
			if(exists $config->{'lexicalsuper-exclude-words'} && 
				$cdi_name_e !~ /\b(tributary|branch) of .+/i &&
				(
					$cdi_name_e =~ /\b(complex|tree|network|set) of .+/i ||
					$cdi_name_e =~ /\b(disk|valve|ligament|artery|vein|duct|mater|joint) of .+/i ||
					$cdi_name_e =~ /.+ of (Giacomini|Guyon|Carabelli|Clarke|Ecker|Calleja|Schwalbe|Zinn)\b/i
				)
			){
				say STDERR qq|exclude:[$cdi_full_name][$cdi_name_e] -> [$cdi_full_pname][$cdi_pname_e]|;
				next;
			...
			$PART_OF{$cdi_full_name}->{$cdi_full_pname}->{$p_type} = undef;

#			if($cdi_name =~ /^0+/ || $cdi_pname =~ /^0+/){
#				$cdi_name =~ s/^0+//g;
#				$cdi_pname =~ s/^0+//g;
#				$cdi_full_name = 'FMA'.$cdi_name;
#				$cdi_full_pname = 'FMA'.$cdi_pname;
...
#			}
		...
		die qq|unknown file $config->{'lexicalsuper'}|;
	}
}
...
SQL
#	$sql =<<SQL;
#UPDATE concept_data SET cd_delcause='DELETE' WHERE ci_id=$ci_id AND cb_id=$cb_id;
#SQL
	print $sql;
...
my $sth_ins_isa    = $dbh->prepare(qq|insert into concept_tree (ci_id,cb_id,cdi_id,cdi_pid,crl_id) values ($ci_id,$cb_id,?,?,3)|);
my $sth_ins_partof = $dbh->prepare(qq|insert into concept_tree (ci_id,cb_id,cdi_id,cdi_pid,crt_ids,crl_id) values ($ci_id,$cb_id,?,?,?,4)|);
