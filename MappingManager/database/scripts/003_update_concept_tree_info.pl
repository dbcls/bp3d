...
INSERT INTO concept_tree_info (ci_id,cb_id,cdi_id,crl_id)
SELECT
 ci_id,cb_id,cdi_id,crl_id
FROM (
 select ci_id,cb_id,cdi_id,crl_id from concept_tree where ci_id=$ci_id AND cb_id=$cb_id AND crl_id=$crl_id
 union
 select ci_id,cb_id,cdi_pid as cdi_id,crl_id from concept_tree where ci_id=$ci_id AND cb_id=$cb_id AND crl_id=$crl_id
) as a
GROUP BY
 ci_id,cb_id,cdi_id,crl_id
SQL
